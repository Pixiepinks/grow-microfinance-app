...
const investorFundingState = { investors: [], agreements: [], accounts: [], settings: {}, lastFundingResult: null, investorOptionsCache: null };
...
function investorApiError(e, resource='funding'){ const message=String(e?.message||''); const code=message.toLowerCase(); if(e?.status===401) return 'Your session has expired. Please sign in again.'; if(e?.status===403) return resource==='investor'?'You do not have permission to perform this investor action.':'You do not have permission to perform this investor funding action.'; if(code.includes('investor_not_found')) return 'The investor was not found.'; if(code.includes('investor_funding_not_found')) return resource==='funding'?'The investor funding record was not found.':'The investor was not found.'; if(e?.status===404){ if(resource==='investor-create' && (!message || code.includes('request failed with status 404'))) return 'The Investor API endpoint was not found.'; if(resource==='investor-options') return 'Investor options endpoint was not found.'; return resource==='funding'?'The investor funding record was not found.':'The investor was not found.'; } if(e?.status===422) return e.message || (resource==='funding'?'The investor funding request contains validation errors.':'The investor request contains validation errors.'); if(e?.status===500) return resource==='funding'?'The server could not process the investor funding request.':'The server could not process the investor request.'; if(e?.name==='AbortError') return resource==='funding'?'Investor funding request timed out. Please retry.':'Investor request timed out. Please retry.'; return e?.message || (resource==='funding'?'Investor funding request failed.':'Investor request failed.'); }
...
function accountMatchesRole(a, role){ const type=String(a?.account_type||a?.type||'').toUpperCase(); const subtype=String(a?.account_subtype||a?.subtype||'').toUpperCase(); const roleText=String(a?.role||a?.account_role||a?.system_role||a?.purpose||'').toUpperCase(); const text=[a?.code,a?.account_code,a?.name,a?.account_name,subtype,roleText].filter(Boolean).join(' ').toUpperCase(); if(role==='funding') return type==='ASSET'&&['BANK','CASH'].includes(subtype); if(role==='liability') return type==='LIABILITY'&&(roleText.includes('INVESTOR')||text.includes('INVESTOR BORROW')||subtype.includes('INVESTOR')); if(role==='expense') return type==='EXPENSE'&&(roleText.includes('INVESTOR')||text.includes('INVESTOR INTEREST')); if(role==='payable') return type==='LIABILITY'&&(roleText.includes('INTEREST')||text.includes('INTEREST PAYABLE'))&&(roleText.includes('INVESTOR')||text.includes('INVESTOR')); if(role==='tax') return type==='LIABILITY'&&(roleText.includes('WITHHOLDING')||text.includes('WITHHOLDING TAX')); return true; }
function agreementAccountOptions(acc, role){ const rows=acc.filter(a=>accountMatchesRole(a,role)); return '<option value="">Select account</option>'+investorOptions(rows, investorAccountLabel); }
function agreementForm(existing={}){ const acc=investorFundingState.accounts; const modal=document.createElement('div'); modal.className='modal-overlay historical-accounting-modal'; const bankOpts=agreementAccountOptions(acc,'funding'), liabilityOpts=agreementAccountOptions(acc,'liability'), expenseOpts=agreementAccountOptions(acc,'expense'), payableOpts=agreementAccountOptions(acc,'payable'), taxOpts=agreementAccountOptions(acc,'tax'); modal.innerHTML=`<div class="modal-card wide"><div class="modal-header"><h2>${existing.id?'Edit':'Create'} Funding Agreement</h2><button class="icon-button" data-close>×</button></div><div id="agreement-msg"><div class="alert info">Loading investors...</div></div><div class="accounting-grid"><label>Investor<select id="ag-investor" disabled><option value="">Loading investors...</option></select><small id="ag-investor-help"></small></label><label>Agreement name<input id="ag-name" value="${escapeHtml(existing.agreement_name||'')}"></label><label>Agreement date<input id="ag-date" type="date" value="${escapeHtml(String(existing.agreement_date||investorToday()).slice(0,10))}"></label><label>Start date<input id="ag-start" type="date" value="${escapeHtml(String(existing.start_date||investorToday()).slice(0,10))}"></label><label>Maturity date<input id="ag-maturity" type="date" value="${escapeHtml(String(existing.maturity_date||'').slice(0,10))}"></label><label>Original expected principal<input id="ag-principal" type="number" step="0.01" value="${escapeHtml(existing.original_expected_principal||existing.original_principal||'')}"></label><label>Interest rate<input id="ag-rate" type="number" step="0.01" value="${escapeHtml(existing.interest_rate||2)}"><small id="ag-rate-display"></small></label><label>Interest rate period<select id="ag-period"><option value="MONTHLY">Monthly</option><option value="ANNUAL">Annual</option></select></label><label>Interest calculation method<select id="ag-method"><option value="MONTHLY_AVERAGE_DAILY_BALANCE">Monthly Average Daily Balance</option><option value="DAILY_BALANCE">Daily Balance</option><option value="FIXED_MONTHLY">Fixed Monthly</option></select></label><label>Interest payment frequency<select id="ag-frequency"><option>MONTHLY</option><option>QUARTERLY</option><option>AT_MATURITY</option></select></label><label>Compounding method<select id="ag-compounding"><option>NONE</option><option>CAPITALIZE_MONTHLY</option></select></label><label>Day-count basis<select id="ag-day"><option>ACTUAL_365</option><option>ACTUAL_360</option><option>30_360</option></select></label><label>Interest payment method<select id="ag-pay-method"><option>BANK_TRANSFER</option><option>CAPITALIZE</option></select></label><label>Funding bank account<select id="ag-bank">${bankOpts}</select></label><label>Investor liability account<select id="ag-liability">${liabilityOpts}</select></label><label>Investor interest expense account<select id="ag-expense">${expenseOpts}</select></label><label>Accrued interest payable account<select id="ag-payable">${payableOpts}</select></label><label>Withholding tax account<select id="ag-tax-account">${taxOpts}</select></label><label>Withholding tax rate<input id="ag-tax" type="number" step="0.01" value="${escapeHtml(existing.withholding_tax_rate||0)}"></label><label>Allow additional funding<select id="ag-add"><option value="true">Yes</option><option value="false">No</option></select></label><label>Allow partial repayment<select id="ag-partial"><option value="true">Yes</option><option value="false">No</option></select></label><label>Auto-accrual enabled<select id="ag-auto"><option value="true">Yes</option><option value="false">No</option></select></label><label>Auto-capitalize interest<select id="ag-cap"><option value="false">No</option><option value="true">Yes</option></select></label><label>Status<select id="ag-status"><option>ACTIVE</option><option>DRAFT</option><option>CLOSED</option></select></label></div><div class="modal-actions sticky-modal-footer"><button class="secondary" data-close>Cancel</button><button id="save-agreement" disabled>Save Agreement</button></div></div>`; document.body.appendChild(modal); modal.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>modal.remove()); const rate=modal.querySelector('#ag-rate'), period=modal.querySelector('#ag-period'), display=modal.querySelector('#ag-rate-display'), saveBtn=modal.querySelector('#save-agreement'), msg=modal.querySelector('#agreement-msg'), investorSelect=modal.querySelector('#ag-investor'), investorHelp=modal.querySelector('#ag-investor-help'); const sync=()=>display.textContent=investorRate(rate.value,period.value); rate.oninput=period.onchange=sync; sync(); const fillInvestors=async()=>{ msg.innerHTML='<div class="alert info">Loading investors...</div>'; investorSelect.disabled=true; saveBtn.disabled=true; investorSelect.innerHTML='<option value="">Loading investors...</option>'; try{ const investors=await loadInvestorAgreementOptions(true); investorSelect.innerHTML=investorAgreementOptionHtml(investors); if(existing.investor_id) investorSelect.value=String(existing.investor_id); investorSelect.disabled=false; saveBtn.disabled=false; msg.innerHTML=''; investorHelp.innerHTML=investors.length?'':'No active investors are available.<br><button type="button" data-add-investor>Add Investor</button>'; }catch(e){ investorHelp.innerHTML=''; msg.innerHTML=`<div class="alert error">Investor list could not be loaded.<br>${escapeHtml(investorApiError(e,'investor-options'))}<br><button type="button" data-retry-investors>Retry</button></div>`; } }; msg.addEventListener('click',e=>{ if(e.target.closest('[data-retry-investors]')) fillInvestors(); }); investorHelp.addEventListener('click',e=>{ if(e.target.closest('[data-add-investor]')) openInvestorForm(); }); fillInvestors(); saveBtn.onclick=async()=>{ const selectedInvestorId=investorSelect.value; if(!selectedInvestorId||!Number.isInteger(Number(selectedInvestorId))){ msg.innerHTML='<div class="alert error">Select an investor.</div>'; return; } const body={investor_id:Number(selectedInvestorId),agreement_name:modal.querySelector('#ag-name').value,agreement_date:modal.querySelector('#ag-date').value,start_date:modal.querySelector('#ag-start').value,maturity_date:modal.querySelector('#ag-maturity').value||null,original_principal_amount:Number(modal.querySelector('#ag-principal').value||0),interest_rate:Number(rate.value||0),interest_rate_period:period.value,calculation_method:modal.querySelector('#ag-method').value,interest_payment_frequency:modal.querySelector('#ag-frequency').value,compounding_method:modal.querySelector('#ag-compounding').value,day_count_basis:modal.querySelector('#ag-day').value,interest_payment_method:modal.querySelector('#ag-pay-method').value,funding_account_id:Number(modal.querySelector('#ag-bank').value),investor_liability_account_id:Number(modal.querySelector('#ag-liability').value),interest_expense_account_id:Number(modal.querySelector('#ag-expense').value),accrued_interest_payable_account_id:Number(modal.querySelector('#ag-payable').value),withholding_tax_account_id:modal.querySelector('#ag-tax-account').value?Number(modal.querySelector('#ag-tax-account').value):null,withholding_tax_rate:Number(modal.querySelector('#ag-tax').value||0),allow_additional_funding:modal.querySelector('#ag-add').value==='true',allow_partial_withdrawal:modal.querySelector('#ag-partial').value==='true',auto_accrual_enabled:modal.querySelector('#ag-auto').value==='true',auto_capitalize_interest:modal.querySelector('#ag-cap').value==='true',status:modal.querySelector('#ag-status').value}; try{ saveBtn.disabled=true; await api(existing.id?`${investorFundingRoutes.agreements}/${encodeURIComponent(existing.id)}`:investorFundingRoutes.agreements,{method:existing.id?'PATCH':'POST',body}); modal.remove(); loadAgreementsPage(); }catch(e){ saveBtn.disabled=false; msg.innerHTML=`<div class="alert error">${escapeHtml(investorApiError(e))}</div>`; } }; }
