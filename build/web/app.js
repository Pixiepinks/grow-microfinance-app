...
function reportBalanceHasActivity(d){ const groups=[d.assets,d.liabilities,d.equity].filter(Boolean); return d.has_activity===true || groups.some(g=>reportAnyRows([g]) || reportAnyRows(g.sections||g.subsections||g.categories||[])) || ['total_assets','total_liabilities','total_equity','total_liabilities_and_equity','difference'].some(k=>reportAmountNumber(d[k]??d.totals?.[k])!==0); }
...
async function loadFinancialPosition(){const root=document.querySelector('#accounting-financial-position-root'); if(!root)return; reportShell(root,'Statement of Financial Position',`<label>As of Date<input name="as_of_date" type="date" value="${reportToday()}"></label><label>Comparative As of Date<input name="comparative_as_of_date" type="date"></label><label><input name="include_zero_balances" type="checkbox" value="true"> Include Zero Balances</label><button type="button" data-export>Export CSV</button>`,'Loading...'); const form=root.querySelector('form'); async function run(e){e&&e.preventDefault(); const box=root.querySelector('.report-results'); try{const d=await api('/admin/accounting/reports/statement-of-financial-position?'+reportQuery(form)); const diff=d.difference??d.balancing_difference??d.totals?.difference??d.totals?.balancing_difference??0; const balanced=d.balanced===true; const hasActivity=reportBalanceHasActivity(d); const totalAssets=d.assets?.total??d.total_assets??d.totals?.total_assets??0; const totalLiabilities=d.liabilities?.total??d.total_liabilities??d.totals?.total_liabilities??0; const totalEquity=d.equity?.total??d.total_equity??d.totals?.total_equity??0; const totalLAndE=d.total_liabilities_and_equity??d.totals?.total_liabilities_and_equity??(reportAmountNumber(totalLiabilities)+reportAmountNumber(totalEquity)); const body=hasActivity?reportTable(['Account Code','Description','Amount'],[reportRenderNestedSections('Assets',d.assets||{total:totalAssets}),`<tr class="total-row"><td colspan="2"><strong>Total Assets</strong></td><td class="money"><strong>${reportCurrency(totalAssets)}</strong></td></tr>`,reportRenderNestedSections('Liabilities',d.liabilities||{total:totalLiabilities}),`<tr class="total-row"><td colspan="2"><strong>Total Liabilities</strong></td><td class="money"><strong>${reportCurrency(totalLiabilities)}</strong></td></tr>`,reportRenderNestedSections('Equity',d.equity||{total:totalEquity}),`<tr class="total-row"><td colspan="2"><strong>Total Equity</strong></td><td class="money"><strong>${reportCurrency(totalEquity)}</strong></td></tr><tr class="total-row"><td colspan="2"><strong>Total Liabilities and Equity</strong></td><td class="money"><strong>${reportCurrency(totalLAndE)}</strong></td></tr><tr class="total-row"><td colspan="2"><strong>Balancing Difference</strong></td><td class="money"><strong>${reportCurrency(diff)}</strong></td></tr>`]):'<p>No balance sheet activity was found up to the selected date.</p>'; box.innerHTML=`<h3>GROW Microfinance — Statement of Financial Position</h3><p>As of ${escapeHtml(form.as_of_date.value||reportToday())}</p><div class="alert ${balanced?'success':'error'}">${balanced?'Balanced':'Not balanced'} — Difference ${reportCurrency(diff)}</div>${reportWarnings(d)}${body}`;}catch(err){box.innerHTML=`<div class="alert error">${escapeHtml(err.message)}</div>`;}} form.onsubmit=run; root.querySelector('[data-export]').onclick=()=>api('/admin/accounting/reports/statement-of-financial-position/export.csv?'+reportQuery(form)); await run();}
