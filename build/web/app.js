...
async function accountingLoadLedger(){ const root=document.querySelector('#accounting-ledger-root'); if(!root)return; if(!accountingState.accounts.length) accountingState.accounts=accountItems(await api('/admin/accounting/accounts?active=true')); root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Accounting</div><h2>General Ledger</h2></div></div><div class="accounting-filters"><select id="ledger-account"><option value="">Account *</option>${accountingState.accounts.map(a=>`<option value="${escapeHtml(a.id)}">${escapeHtml((a.code||a.account_code)+' '+(a.name||a.account_name))}</option>`).join('')}</select><input id="ledger-from" type="date"><input id="ledger-to" type="date"><input id="ledger-customer" placeholder="Customer"><input id="ledger-loan" placeholder="Loan (select account first)" disabled><button id="run-ledger" disabled>Run</button>${accountingCan('accounting.export')?'<button id="export-ledger">Export CSV</button>':''}</div><p class="muted">Select an account to view its ledger transactions.</p><div id="ledger-results"><p>No ledger transactions found for the selected filters.</p></div>`; document.querySelector('#ledger-account').onchange=()=>{const has=!!document.querySelector('#ledger-account').value;document.querySelector('#run-ledger').disabled=!has;document.querySelector('#ledger-loan').disabled=!has;document.querySelector('#ledger-loan').placeholder=has?'Loan':'Loan (account required)';}; document.querySelector('#run-ledger').onclick=async()=>{const id=document.querySelector('#ledger-account').value;if(!id)return; const data=await api(`/admin/accounting/general-ledger?account=${encodeURIComponent(id)}`); const summary=data.summary||data.account||{}; const tx=accountItems(data); document.querySelector('#ledger-results').innerHTML=`<div class="accounting-grid">${['account_code','account_name','account_type','normal_balance'].map(k=>`<div class="metric"><div class="metric-label">${k}</div><div>${escapeHtml(summary[k]||'')}</div></div>`).join('')}${['opening_balance','total_debit','total_credit','closing_balance'].map(k=>`<div class="metric"><div class="metric-label">${k}</div><div>${formatCurrency(summary[k])}</div></div>`).join('')}</div><div class="table-scroll"><table><thead><tr>${['Date','Journal No.','Description','Reference','Customer','Loan','Debit','Credit','Running Balance'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${tx.map(r=>`<tr><td>${escapeHtml(r.date||'')}</td><td><button class="link-button" data-journal-id="${escapeHtml(r.journal_id||r.id)}">${escapeHtml(r.journal_no||'')}</button></td><td>${escapeHtml(r.description||'')}</td><td>${escapeHtml(r.reference||r.reference_id||'')}</td><td>${escapeHtml(r.customer||'')}</td><td>${escapeHtml(r.loan||'')}</td><td>${moneyCell(r.debit)}</td><td>${moneyCell(r.credit)}</td><td>${moneyCell(r.running_balance)}</td></tr>`).join('')}</tbody></table></div>`;}; }
