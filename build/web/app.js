...
async function accountingLoadSettings(){const root=document.querySelector('#accounting-settings-root'); if(!root)return; root.innerHTML='<h2>Accounting Settings</h2><p>Loading...</p>'; try{const [settingsRaw,accountsRaw]=await Promise.all([api('/admin/accounting/settings'),api('/admin/accounting/accounts')]); const accounts=accountItems(accountsRaw); const can=accountingCan('accounting.settings.manage'); const sections={}; accountingSettingFields.forEach(f=>(sections[f[2]] ||= []).push(f)); const valueFor=k=>settingsRaw[k]||settingsRaw[k.replace('_account_id','')]||''; root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Accounting</div><h2>Accounting Settings</h2><p class="muted">Configure account mappings for future transactions.</p></div>${can?'':'<span class="badge">Read-only</span>'}</div><div id="settings-message"></div>${Object.entries(sections).map(([name,fields])=>`<div class="subcard"><h3>${escapeHtml(name)}</h3><div class="accounting-grid">${fields.map(f=>{const [key,label,,types,subs]=f; const eligible=accounts.filter(a=>types.includes(String(acctType(a)).toUpperCase())&&(!subs.length||subs.map(s=>s.replace(/ /g,'_')).includes(String(acctSubtype(a)).toUpperCase().replace(/ /g,'_')))); const selected=accounts.find(a=>String(a.id)===String(valueFor(key))); return `<label>${escapeHtml(label)}<select data-setting-key="${key}" ${can?'':'disabled'}><option value="">Select account</option>${selected&&!eligible.find(a=>String(a.id)===String(selected.id))?`<option selected value="${escapeHtml(selected.id)}">${escapeHtml(accountLabel(selected))} (invalid)</option>`:''}${eligible.map(a=>`<option value="${escapeHtml(a.id)}" ${String(a.id)===String(valueFor(key))?'selected':''}>${escapeHtml(accountLabel(a))}</option>`).join('')}</select><small>Account Type: ${escapeHtml(selected?acctType(selected):'—')}<br>Subtype: ${escapeHtml(selected?acctSubtype(selected):'—')}</small>${selected&&(selected.active===false||selected.is_active===false)?'<div class="alert warning">Warning: this account is inactive.</div>':''}</label>`}).join('')}</div></div>`).join('')}${can?'<div class="action-row"><button id="save-accounting-settings">Save Changes</button><button class="secondary" id="reset-accounting-settings">Reset Unsaved Changes</button></div>':''}`; if(can){ const original={}; accountingSettingFields.forEach(f=>original[f[0]]=String(valueFor(f[0])||'')); document.querySelector('#reset-accounting-settings').onclick=()=>accountingLoadSettings(); document.querySelector('#save-accounting-settings').onclick=async()=>{const changed={}; root.querySelectorAll('[data-setting-key]').forEach(el=>{if(String(el.value||'')!==original[el.dataset.settingKey]) changed[el.dataset.settingKey]=el.value;}); if(!Object.keys(changed).length)return; const summary=Object.entries(changed).map(([k,v])=>`${accountingSettingFields.find(f=>f[0]===k)?.[1]||k}: ${accountLabel(accounts.find(a=>String(a.id)===String(v))||{})}`).join('\n'); if(!confirm(`You are changing accounting account mappings. Future transactions will use\nthe new accounts. Existing posted journals will not be changed.\n\n${summary}`))return; try{await api('/admin/accounting/settings',{method:'PUT',body:changed}); document.querySelector('#settings-message').innerHTML='<div class="alert success">Accounting settings updated successfully.</div>'; }catch(e){document.querySelector('#settings-message').innerHTML=`<div class="alert error">${escapeHtml(e.message||'Failed to update accounting settings.')}</div>`;}};}}catch(e){root.innerHTML=`<div class="alert error">${escapeHtml(e.message)}</div>`;}}
...
async function accountingLoadJournals(){ const root=document.querySelector('#accounting-journals-root'); if(!root)return; root.innerHTML='<h2>Journal Entries</h2><p>Loading...</p>'; try{ const rows=accountItems(await api('/admin/accounting/journals')); accountingState.journals=rows; root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Accounting</div><h2>Journal Entries</h2></div>${accountingCan('accounting.journals.create')?'<button data-accounting-section="accounting-journal-form">New Journal Entry</button>':''}</div><div class="accounting-filters">${['Date From','Date To','Status','Reference Type','Account','Customer','Loan','Search'].map(x=>`<input placeholder="${x}">`).join('')}<button>Apply Filters</button></div><div class="table-scroll"><table><thead><tr>${['Journal No.','Date','Description','Reference Type','Customer','Loan','Status','Total Debit','Total Credit','Created By','Actions'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.map(j=>`<tr><td><button class="link-button" data-journal-id="${escapeHtml(j.id)}">${escapeHtml(j.journal_no||j.number||j.id)}</button></td><td>${escapeHtml(j.date||j.journal_date||'')}</td><td>${escapeHtml(j.description||'')}</td><td>${escapeHtml(refLabel(j.reference_type))}</td><td>${customerDisplay(j)}</td><td>${escapeHtml(loanDisplay(j))}</td><td><span class="badge">${escapeHtml(j.status||'DRAFT')}</span></td><td>${moneyCell(j.total_debit)}</td><td>${moneyCell(j.total_credit)}</td><td>${escapeHtml(j.created_by||'')}</td><td><button data-journal-id="${escapeHtml(j.id)}">View</button>${j.status==='DRAFT'&&accountingCan('accounting.journals.post')?' <button data-post-journal="'+escapeHtml(j.id)+'">Post</button>':''}${j.status==='POSTED'&&accountingCan('accounting.journals.reverse')?' <button data-reverse-journal="'+escapeHtml(j.id)+'">Reverse</button>':''}</td></tr>`).join('')}</tbody></table></div>`;}catch(e){root.innerHTML=`<div class="alert error">${escapeHtml(e.message)}</div>`;} }
