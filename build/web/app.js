...
async function recordAdminLedgerPayment(entryId) {
  ...
  const [settingsRaw, accountsRaw, collectorsRaw] = await Promise.allSettled([
    ...
    api('/admin/collections/collectors/options'),
  ]);
  ...
  const collAccountFor=c=>accounts.find(a=>String(a.id)===String(c.default_collection_account_id||c.defaultCollectionAccountId||c.collection_account_id||c.collectionAccountId)) || accounts.find(a=>isCollection(a)&&String(a.collector_id||a.collectorId||'')===String(c.id)) || accounts.find(a=>isCollection(a)&&String(a.name||a.account_name||'').toLowerCase().includes(String(c.name||c.full_name||'').toLowerCase()));
  const opts=(arr,sel='')=>arr.map(a=>`<option value="${escapeHtml(a.id)}" ${String(a.id)===String(sel)?'selected':''}>${escapeHtml(accountName(a))}</option>`).join('');
  ...
  const validate=()=>{ const amount=Number(amountEl.value||0); let error=''; let warning=''; const historical=isHistoricalDate(dateEl.value); if(disbursementDate && dateOnlyToEpoch(dateEl.value)<dateOnlyToEpoch(disbursementDate)) error='Payment date cannot be earlier than disbursement date.'; else if(isFutureDateOnly(dateEl.value)) error='Future payment dates are not supported.'; else if(historical && String(settings.allow_historical_collections ?? settings.allowHistoricalCollections ?? settings.allow_backdated_payment ?? true)==='false') error='Historical collections are disabled by accounting settings.'; if(historical){ const locked=String(settings.locked_period_posting||'').toUpperCase()==='BLOCK' || String(settings.accounting_period_locked||settings.periodLocked)==='true'; if(locked) error='Backend error: selected accounting period is locked.'; warning=`<div class="alert warning"><strong>Historical payment</strong><br>This payment will be posted using the selected historical accounting date.<div class="accounting-grid"><div><strong>Loan disbursement date</strong><br>${escapeHtml(formatDateOnlyDisplay(disbursementDate)||'—')}</div><div><strong>Selected payment date</strong><br>${escapeHtml(formatDateOnlyDisplay(dateEl.value))}</div><div><strong>Interest accruals required through payment date</strong><br>${escapeHtml(settings.historical_payments_auto_accrue===false?'Manual accrual required':'Will be calculated by backend')}</div><div><strong>Accounting-period status</strong><br>${escapeHtml(periodStatus())}</div></div></div>`; }
    hist.innerHTML=warning; const interest=Math.min(amount, Number(getLedgerField(ledgerEntry, ['interest','interest_amount','interestAmount'], amount*0.2))||0); const principal=Math.max(0, amount-interest); const selectedAccount = methodEl.value==='BANK_TRANSFER' ? accounts.find(a=>String(a.id)===bankEl.value) : accounts.find(a=>String(a.id)===accountEl.value); const collectorCash=methodEl.value==='CASH_COLLECTOR'; preview.innerHTML=`<h3>Allocation Preview</h3><p><strong>Payment:</strong> ${formatCurrency(amount)}</p><ul><li>Delay interest: ${formatCurrency(0)}</li><li>Interest: ${formatCurrency(interest)}</li><li>Principal: ${formatCurrency(principal)}</li><li>Unapplied: ${formatCurrency(0)}</li></ul>${collectorCash?`<div class="alert warning"><strong>Cash destination:</strong> ${escapeHtml(accountName(selectedAccount)||'Collection Account')}<br>This payment will be posted to ${escapeHtml((accountName(selectedAccount)||'Collection Account').replace(/^\d+\s*[—-]?\s*/,''))} until deposited to a company bank account.</div>`:''}`; if(methodEl.value==='BANK_TRANSFER'&&!bankEl.value) error='Select a bank account for a direct transfer.'; if(methodEl.value==='CASH_COLLECTOR'&&!collectorEl.value) error='Select a collector.'; if(methodEl.value==='CASH_COLLECTOR'&&collectorEl.value&&!accountEl.value) error='This collector has no active collection account.'; if(methodEl.value!=='BANK_TRANSFER'&&selectedAccount&&isControlAccount(selectedAccount)) error='The control account cannot be used for customer payments.'; if(methodEl.value!=='BANK_TRANSFER'&&!accountEl.value) error=methodEl.value==='CASH_COLLECTOR'?error:'Select a collection account.'; if(error) err.innerHTML=`<div class="alert error">${escapeHtml(error)}</div>`; else err.innerHTML=''; btn.disabled=!!error || !(amount>0 && dateEl.value && methodEl.value); };
  modal.querySelector('[data-setup-collector]')?.addEventListener('click',()=>{ modal.remove(); showAdminSection('collections-collectors'); setTimeout(()=>openCollectorSetupWizard({fromPayment:true}),50); }); [dateEl,amountEl,methodEl,accountEl,bankEl,bankRefEl,refEl,remarksEl].forEach(el=>el.addEventListener('input',validate)); methodEl.addEventListener('change',applyMethod); applyMethod(); validate();
  btn.onclick=async()=>{ if(btn.disabled)return; const method=methodEl.value; btn.disabled=true; err.innerHTML=''; try{ const body={ paid_amount:Number(amountEl.value), amount:Number(amountEl.value), paid_date:dateEl.value, payment_date:dateEl.value, accounting_date:dateEl.value, collection_method:method, collector_id:collectorEl.value||undefined, collection_account_id: method==='BANK_TRANSFER'?bankEl.value:accountEl.value, bank_account_id: method==='BANK_TRANSFER'?bankEl.value:undefined, bank_reference:bankRefEl.value.trim(), reference:refEl.value.trim()||bankRefEl.value.trim(), remarks:remarksEl.value.trim() }; const res=await api(`/admin/loans/${encodeURIComponent(loanId)}/ledger/${encodeURIComponent(entryId)}/payment`, { method:'POST', body }); const account=method==='BANK_TRANSFER'?accounts.find(a=>String(a.id)===bankEl.value):accounts.find(a=>String(a.id)===accountEl.value); modal.querySelector('.modal-card').innerHTML=`<div class="modal-header"><h2>Collection Receipt Result</h2><button class="icon-button" data-close-success>×</button></div><div class="loan-detail-grid">${[['Receipt number',res.receipt_number||res.receiptNo||res.id||'—'],['Customer',res.customer_name||customerDisplay(loan)||'—'],['Loan',res.loan_number||getLoanField(loan,['loan_number','loanNumber'],'—')],['Collector',collectorEl.options[collectorEl.selectedIndex]?.text||'—'],['Collection method',refLabel(method)],['Collection account',accountName(account)||'—'],['Payment date',formatDateOnlyDisplay(dateEl.value)],['Amount',formatCurrency(amountEl.value)],['Principal allocation',formatCurrency(res.principal_allocation||res.principalAllocation||0)],['Interest allocation',formatCurrency(res.interest_allocation||res.interestAllocation||0)],['Deposit status',method==='CASH_COLLECTOR'?'Undeposited':(res.deposit_status||'Not required')]].map(([l,v])=>`<div class="loan-detail-stat"><span>${escapeHtml(l)}</span><strong>${escapeHtml(String(v))}</strong></div>`).join('')}</div><button data-close-success>Close</button>`; modal.querySelectorAll('[data-close-success]').forEach(b=>b.onclick=()=>modal.remove()); setInlineAlert(adminLoanDetailMessage,'Payment recorded successfully.','success'); await loadAdminLoanLedger(true); await loadAdminLoans(true); }catch(error){ console.error('Failed to record ledger payment', error); err.innerHTML=`<div class="alert error">${escapeHtml(error?.message || 'Failed to record payment.')}</div>`; }finally{ validate(); } };
}
...
async function collectionBootstrapData(){
  const [settings, accounts, collectors] = await Promise.allSettled([api('/admin/accounting/settings'), api('/admin/accounting/accounts?active=true'), api('/admin/collections/collectors/options')]);
...
async function loadDepositCollections(){ const root=document.querySelector('#collections-deposit-root'); if(!root)return; root.innerHTML='<h2>Deposit Collections</h2><p>Loading...</p>'; try{ await collectionBootstrapData(); const collectors=collectionState.collectors, accounts=collectionState.accounts; const allowPartial=String(collectionState.settings.allow_partial_deposits ?? collectionState.settings.allowPartialDeposits ?? true)!=='false'; root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Collections</div><h2>Deposit Collections</h2></div></div><div id="deposit-message"></div><div class="accounting-grid"><label>Collector<select id="deposit-collector"><option value="">Select collector</option>${collectors.map(c=>`<option value="${escapeHtml(c.id)}">${escapeHtml(collectionCollectorName(c))}</option>`).join('')}</select></label><label>Collector collection account<select id="deposit-source-account"><option value="">Select account</option>${accounts.filter(collectionIsClearing).map(a=>`<option value="${escapeHtml(a.id)}">${escapeHtml(collectionAccountLabel(a))}</option>`).join('')}</select></label><label>Deposit date<input id="deposit-date" type="date" value="${todayDateOnly()}"></label><label>Destination bank account<select id="deposit-bank"><option value="">Select bank account</option>${accounts.filter(collectionIsBank).map(a=>`<option value="${escapeHtml(a.id)}">${escapeHtml(collectionAccountLabel(a))}</option>`).join('')}</select></label><label>Bank deposit/reference number<input id="deposit-reference"></label><label>Remarks<textarea id="deposit-remarks"></textarea></label></div><div id="historical-deposit-panel"></div><div id="deposit-receipts"><p class="muted">Select a collector to load all undeposited collections.</p></div><div id="deposit-preview" class="subcard"></div><button id="post-collection-deposit" class="sticky-modal-footer" disabled>Post Collection Deposit</button>`; const collectorEl=root.querySelector('#deposit-collector'), sourceEl=root.querySelector('#deposit-source-account'), dateEl=root.querySelector('#deposit-date'), bankEl=root.querySelector('#deposit-bank'), refEl=root.querySelector('#deposit-reference'), receiptsEl=root.querySelector('#deposit-receipts'), preview=root.querySelector('#deposit-preview'), btn=root.querySelector('#post-collection-deposit'), msg=root.querySelector('#deposit-message'); let rows=[]; const total=()=>[...root.querySelectorAll('[data-deposit-line]:checked')].reduce((s,cb)=>s+Number(root.querySelector(`[data-include-amount="${CSS.escape(cb.value)}"]`)?.value||cb.dataset.undeposited||0),0); const renderPreview=()=>{ const bank=accounts.find(a=>String(a.id)===bankEl.value), src=accounts.find(a=>String(a.id)===sourceEl.value); const t=total(); const hist=isHistoricalDate(dateEl.value); root.querySelector('#historical-deposit-panel').innerHTML=hist?`<div class="alert warning"><strong>Historical collector deposit</strong><br>Validate deposit date is not earlier than selected payment dates, accounting period is open, and sufficient collector balance existed on that date. Do not silently use today’s date.</div>`:''; preview.innerHTML=`<h3>Deposit journal preview</h3><div class="accounting-grid"><p><strong>Dr ${escapeHtml(collectionAccountLabel(bank)||'Main Bank Account')}</strong><br>${formatCurrency(t)}</p><p><strong>Cr ${escapeHtml(collectionAccountLabel(src)||'Collection Account – Collector')}</strong><br>${formatCurrency(t)}</p></div><p class="muted">This deposit transfers collector-held cash to the bank. Customer loan balances will not be changed again.</p><p><strong>Remaining collector balance after deposit:</strong> ${formatCurrency(rows.reduce((s,r)=>s+Number(r.undeposited_amount??r.amount??0),0)-t)}</p>`; btn.disabled=!(collectorEl.value&&sourceEl.value&&bankEl.value&&t>0&&dateEl.value&&(!collectionState.settings.require_bank_reference_for_deposit||refEl.value.trim())); }; const renderRows=()=>{ receiptsEl.innerHTML=`<h3>Selected customer collections</h3><div class="table-scroll collection-responsive-table"><table><thead><tr><th>Select</th><th>Receipt</th><th>Customer</th><th>Payment date</th><th>Undeposited amount</th><th>Amount included in this deposit</th><th>Remaining</th></tr></thead><tbody>${rows.map(r=>{const id=r.id||r.receipt_id||r.receipt_number; const und=Number(r.undeposited_amount??r.amount??0); return `<tr><td><input type="checkbox" data-deposit-line value="${escapeHtml(id)}" data-undeposited="${und}"></td><td>${escapeHtml(r.receipt_number||id)}</td><td>${escapeHtml(r.customer_name||'—')}</td><td>${escapeHtml(formatDateOnlyDisplay(r.payment_date))}</td><td>${formatCurrency(und)}</td><td><input ${allowPartial?'':'readonly'} data-include-amount="${escapeHtml(id)}" type="number" step="0.01" max="${und}" value="${und}"></td><td data-remaining="${escapeHtml(id)}">${formatCurrency(0)}</td></tr>`;}).join('')}</tbody></table></div><div class="subcard sticky-modal-footer">Selected total: <strong id="deposit-selected-total">${formatCurrency(0)}</strong></div>`; receiptsEl.querySelectorAll('input').forEach(i=>i.oninput=()=>{ receiptsEl.querySelectorAll('[data-include-amount]').forEach(inp=>{ const row=rows.find(r=>String(r.id||r.receipt_id||r.receipt_number)===String(inp.dataset.includeAmount)); const und=Number(row?.undeposited_amount??row?.amount??0); const rem=Math.max(0,und-Number(inp.value||0)); receiptsEl.querySelector(`[data-remaining="${CSS.escape(inp.dataset.includeAmount)}"]`).textContent=formatCurrency(rem); }); receiptsEl.querySelector('#deposit-selected-total').textContent=formatCurrency(total()); renderPreview(); }); renderPreview(); }; collectorEl.onchange=async()=>{ receiptsEl.innerHTML='Loading undeposited collections...'; rows=collectionItems(await api(`/admin/collections/undeposited?collector_id=${encodeURIComponent(collectorEl.value)}`)); renderRows(); }; [sourceEl,dateEl,bankEl,refEl].forEach(el=>el.oninput=renderPreview); btn.onclick=()=>{ const selected=[...root.querySelectorAll('[data-deposit-line]:checked')]; const t=total(); if(!confirm(`Post Collection Deposit\n\nCollector: ${collectorEl.options[collectorEl.selectedIndex]?.text}\nDestination: ${bankEl.options[bankEl.selectedIndex]?.text}\nReceipts: ${selected.length}\nDeposit total: ${formatCurrency(t)}\n\nJournal:\nDr ${bankEl.options[bankEl.selectedIndex]?.text}\nCr ${sourceEl.options[sourceEl.selectedIndex]?.text}`))return; api('/admin/collections/deposits',{method:'POST',body:{collector_id:collectorEl.value,collection_account_id:sourceEl.value,deposit_date:dateEl.value,bank_account_id:bankEl.value,bank_reference:refEl.value.trim(),remarks:root.querySelector('#deposit-remarks').value.trim(),allocations:selected.map(cb=>({receipt_id:cb.value,amount:Number(root.querySelector(`[data-include-amount="${CSS.escape(cb.value)}"]`).value||0)})),deposit_amount:t}}).then(()=>{msg.innerHTML='<div class="alert success">Collection deposit posted.</div>'; loadCollectorBalances();}).catch(e=>msg.innerHTML=`<div class="alert error">${escapeHtml(e.message)}</div>`); }; }catch(e){root.innerHTML=`<div class="alert error">${escapeHtml(e.message)}</div>`;} }
async function loadDepositRegister(){ const root=document.querySelector('#collections-register-root'); if(!root)return; root.innerHTML='<h2>Deposit Register</h2><p>Loading...</p>'; try{ const rows=collectionItems(await api('/admin/collections/deposits')); root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Collections</div><h2>Deposit Register</h2></div></div><div class="table-scroll"><table><thead><tr>${['Deposit number','Deposit date','Collector','Collector account','Bank account','Amount','Bank reference','Status','Actions'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr><td>${escapeHtml(r.deposit_number||r.id)}</td><td>${escapeHtml(formatDateOnlyDisplay(r.deposit_date))}</td><td>${escapeHtml(collectionCollectorName(r))}</td><td>${escapeHtml(r.collection_account_name||'—')}</td><td>${escapeHtml(r.bank_account_name||'—')}</td><td>${formatCurrency(r.amount||r.deposit_amount)}</td><td>${escapeHtml(r.bank_reference||'—')}</td><td><span class="badge">${escapeHtml(r.status||'Posted')}</span></td><td><button data-view-deposit="${escapeHtml(r.id)}">View</button> <button data-reverse-deposit="${escapeHtml(r.id)}">Reverse</button> <button onclick="window.print()">Print Deposit Summary</button></td></tr>`).join('')}</tbody></table></div>`; }catch(e){root.innerHTML=`<div class="alert error">${escapeHtml(e.message)}</div>`;} }
...
function collectorAccountName(c){return c?.collection_account_name||c?.collectionAccountName||c?.default_collection_account_name||c?.defaultCollectionAccountName||'—';}
...
async function openCollectorSetupWizard(opts={}){
  ...
  const sync=()=>{const opt=staffEl.options[staffEl.selectedIndex]; const name=opt?.dataset.name||'Sanjana'; if(!codeEl.value) codeEl.value='COL-'+name.toUpperCase().replace(/[^A-Z0-9]+/g,'-').replace(/^-|-$/g,''); preview.textContent=`1051 Collection Account – ${name}`;}; staffEl.onchange=sync; searchEl.oninput=()=>{const q=searchEl.value.toLowerCase(); [...staffEl.options].forEach((o,i)=>{if(i)o.hidden=!o.textContent.toLowerCase().includes(q);});}; sync();
  modal.querySelector('#save-collector').onclick=async()=>{const msg=modal.querySelector('#collector-wizard-message'); const name=staffEl.options[staffEl.selectedIndex]?.dataset.name||'Collector'; if(!staffEl.value){msg.innerHTML='<div class="alert error">Select an existing staff member.</div>';return;} try{await api('/admin/collections/collectors',{method:'POST',body:{staff_id:staffEl.value,collector_code:codeEl.value.trim(),can_collect_cash:modal.querySelector('#collector-can-cash').value==='true',status:modal.querySelector('#collector-status').value,create_collection_account:modal.querySelector('#collector-auto-account').value==='true'}}); msg.innerHTML=`<div class="alert success">Collector ${escapeHtml(name)} was enabled and Collection Account – ${escapeHtml(name)} was created.</div>`; await collectionBootstrapData(); await loadCollectorsManagement(); setTimeout(()=>modal.remove(),900);}catch(e){msg.innerHTML=`<div class="alert error">${escapeHtml(e.message||'Failed to save collector.')}</div>`;}};
}
