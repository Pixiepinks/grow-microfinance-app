...
async function recordAdminLedgerPayment(entryId) {
  ...
  const modal=document.createElement('div'); modal.className='modal-overlay historical-accounting-modal';
  modal.innerHTML=`<div class="modal-card wide"><div class="modal-header"><h2>Record Payment</h2><button class="icon-button" data-close>×</button></div><div id="payment-entry-error"></div><div class="accounting-grid"><label>Payment date<input id="ledger-payment-date" type="date" value="${todayDateOnly()}"></label><label>Collection method<select id="ledger-payment-method"><option>CASH</option><option>BANK_TRANSFER</option><option>CHEQUE</option><option>OTHER</option></select></label><label>Collection account<input id="ledger-payment-account" placeholder="Collection account ID"></label><label>Amount<input id="ledger-payment-amount" type="number" step="0.01" min="0"></label><label>Reference<input id="ledger-payment-reference"></label><label>Remarks<textarea id="ledger-payment-remarks"></textarea></label></div><div id="payment-allocation-preview" class="subcard"></div><div class="modal-actions sticky-modal-footer"><button class="secondary" data-close>Cancel</button><button id="ledger-payment-confirm" disabled>Record Payment</button></div></div>`;
  document.body.appendChild(modal); modal.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>modal.remove());
  const dateEl=modal.querySelector('#ledger-payment-date'), amountEl=modal.querySelector('#ledger-payment-amount'), methodEl=modal.querySelector('#ledger-payment-method'), accountEl=modal.querySelector('#ledger-payment-account'), refEl=modal.querySelector('#ledger-payment-reference'), remarksEl=modal.querySelector('#ledger-payment-remarks'), preview=modal.querySelector('#payment-allocation-preview'), err=modal.querySelector('#payment-entry-error'), btn=modal.querySelector('#ledger-payment-confirm');
  const validate=()=>{ const amount=Number(amountEl.value||0); let html=''; if(disbursementDate && dateOnlyToEpoch(dateEl.value)<dateOnlyToEpoch(disbursementDate)) html='<div class="alert error">Payment date cannot be earlier than disbursement date.</div>'; else if(isFutureDateOnly(dateEl.value)) html='<div class="alert error">Future payment dates are not supported.</div>'; else if(isHistoricalDate(dateEl.value)) html='<div class="alert warning"><strong>Historical payment</strong><br>Required interest accruals up to this payment date will be posted before allocating the payment.</div>'; const interest=Math.min(amount, Number(getLedgerField((adminLoansState.ledger||[]).find(e=>String(getLedgerField(e,['id','entry_id','entryId']))===String(entryId))||{}, ['interest','interest_amount','interestAmount'], amount*0.2))||0); const principal=Math.max(0, amount-interest); preview.innerHTML=`${html}<h3>Allocation Preview</h3><p>Payment: ${formatCurrency(amount)}</p><ul><li>Delay interest: ${formatCurrency(0)}</li><li>Interest: ${formatCurrency(interest)}</li><li>Principal: ${formatCurrency(principal)}</li><li>Unapplied: ${formatCurrency(0)}</li></ul>`; btn.disabled=!!html.includes('alert error') || !(amount>0 && dateEl.value && methodEl.value); };
  [dateEl,amountEl,methodEl,accountEl,refEl,remarksEl].forEach(el=>el.addEventListener('input',validate)); validate();
  btn.onclick=async()=>{ if(btn.disabled)return; btn.disabled=true; err.innerHTML=''; try{ await api(`/admin/loans/${encodeURIComponent(loanId)}/ledger/${encodeURIComponent(entryId)}/payment`, { method:'POST', body:{ paid_amount:Number(amountEl.value), amount:Number(amountEl.value), paid_date:dateEl.value, payment_date:dateEl.value, accounting_date:dateEl.value, collection_method:methodEl.value, collection_account_id:accountEl.value.trim(), reference:refEl.value.trim(), remarks:remarksEl.value.trim() } }); setInlineAlert(adminLoanDetailMessage,'Payment recorded successfully.','success'); modal.remove(); await loadAdminLoanLedger(true); await loadAdminLoans(true); }catch(error){ console.error('Failed to record ledger payment', error); err.innerHTML=`<div class="alert error">${escapeHtml(error?.message || 'Failed to record payment.')}</div>`; }finally{ btn.disabled=false; validate(); } };
}
...
async function accountingLoadSettings(){
  ...
  try{
    ...
    const operationalFields=[
      ...
      ['allow_backdated_payment','Allow Backdated Payment','boolean',null,'Permit repayment collection with past accounting dates when period controls allow it.'],
