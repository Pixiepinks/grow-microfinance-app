...
function getGrossLoanPrincipal(record = {}) {
  return moneyNumber(getLoanField(record, ['gross_principal','grossPrincipal','approved_principal','approvedPrincipal','principal_amount','principalAmount','principal','amount','approved_amount','approvedAmount','applied_amount','appliedAmount'], 0));
}

function normalizeDisbursementCharges(source = {}) {
  const candidates = [source.disbursement_charges, source.disbursementCharges, source.charges, source.default_charges, source.defaultCharges, source.data?.charges, source.data?.items, source.items, source.data];
  const arr = candidates.find(Array.isArray) || [];
  return arr.map((c, i) => ({
    uid: c.uid || c.id || `charge-${Date.now()}-${i}`,
    code: c.code || c.charge_code || c.chargeCode || '',
    active: c.active ?? c.is_active ?? c.isActive ?? true,
    refundable: boolFromBackend(c.refundable ?? c.is_refundable, false),
    deduct_from_disbursement: c.deduct_from_disbursement ?? c.deductFromDisbursement ?? true,
    charge_type_id: c.charge_type_id ?? c.chargeTypeId ?? c.type_id ?? c.id ?? '',
    charge_type_name: c.charge_type_name || c.chargeTypeName || c.name || c.charge_name || c.chargeName || 'Deduction',
    calculation_method: c.calculation_method || c.calculationMethod || 'FIXED_AMOUNT',
    rate: moneyNumber(c.rate ?? c.default_rate ?? c.defaultRate, 0),
    amount: moneyNumber(c.amount ?? c.default_amount ?? c.defaultAmount ?? c.gross_amount ?? c.grossAmount, 0),
    tax_method: c.tax_method || c.taxMethod || 'NO_TAX',
    tax_rate: moneyNumber(c.tax_rate ?? c.taxRate, 0),
    accounting_treatment: c.accounting_treatment || c.accountingTreatment || c.treatment || 'INCOME',
    gl_account_id: c.gl_account_id ?? c.glAccountId ?? c.destination_gl_account_id ?? c.destinationGlAccountId ?? c.accounting_destination_id ?? '',
    gl_account_name: c.gl_account_name || c.glAccountName || c.destination_gl_account_name || c.destinationGlAccountName || c.account_name || c.accountName || '',
    description: c.description || '',
    mandatory: boolFromBackend(c.mandatory ?? c.is_mandatory ?? c.required, false),
    waived: boolFromBackend(c.waived ?? c.is_waived, false),
    status: c.status || '',
  }));
}

function disbursementSummaryFromLoan(loan = {}) {
  const summary = loan.disbursement_summary || loan.disbursementSummary || loan.disbursement || {};
  const charges = normalizeDisbursementCharges(summary).length ? normalizeDisbursementCharges(summary) : normalizeDisbursementCharges(loan);
  const gross = moneyNumber(summary.gross_principal ?? summary.grossPrincipal ?? getGrossLoanPrincipal(loan), 0);
  const deductions = moneyNumber(summary.total_deductions ?? summary.totalDeductions, charges.reduce((a,c)=>a+(c.waived?0:moneyNumber(c.amount)),0));
  return {
    gross,
    deductions,
    net: moneyNumber(summary.net_disbursed ?? summary.netDisbursed ?? summary.net_amount ?? summary.netAmount, gross - deductions),
    fundingAccount: summary.funding_account_name || summary.fundingAccountName || loan.funding_account_name || loan.fundingAccountName || '—',
    date: summary.disbursement_date || summary.disbursementDate || loan.disbursement_date || loan.disbursementDate || loan.start_date || loan.startDate || '',
    journal: summary.journal_number || summary.journalNumber || summary.journal_no || summary.journalNo || loan.journal_number || loan.journalNumber || '—',
    charges,
    reversed: boolFromBackend(summary.reversed ?? loan.disbursement_reversed ?? loan.disbursementReversed, false),
    reversalJournal: summary.reversal_journal_number || summary.reversalJournalNumber || loan.reversal_journal_number || '—',
    reversalReason: summary.reversal_reason || summary.reversalReason || loan.reversal_reason || '—',
    reversalDate: summary.reversal_date || summary.reversalDate || loan.reversal_date || '',
  };
}

function renderDisbursementJournalPreviewHtml(preview = {}, gross = 0, net = 0, charges = [], fundingName = 'Selected Bank/Cash Account') {
  const lines = preview.journal_entries || preview.journalEntries || preview.journal_lines || preview.journalLines || [];
  const debitLines = lines.filter(l => String(l.type || l.entry_type || l.dc || '').toUpperCase().startsWith('D'));
  const creditLines = lines.filter(l => String(l.type || l.entry_type || l.dc || '').toUpperCase().startsWith('C'));
  const fallbackDebit = debitLines.length ? debitLines : [{ account_name: 'Loan Principal Receivable', amount: gross }];
  const fallbackCredit = creditLines.length ? creditLines : [{ account_name: fundingName, amount: net }, ...charges.filter(c=>!c.waived).map(c=>({ account_name: c.gl_account_name || c.charge_type_name, amount: c.amount }))];
  const amt = l => moneyNumber(l.amount ?? l.debit ?? l.credit, 0);
  const totalDebit = fallbackDebit.reduce((a,l)=>a+amt(l),0);
  const totalCredit = fallbackCredit.reduce((a,l)=>a+amt(l),0);
  const row = l => `<li><span>${escapeHtml(l.account_name || l.accountName || l.gl_account || l.account || 'Account')}</span><strong>${formatCurrency(amt(l))}</strong></li>`;
  return `<div class="subcard"><h3>Disbursement Journal Preview</h3><div class="accounting-grid"><div><strong>Debit</strong><ul class="plain-list">${fallbackDebit.map(row).join('')}</ul></div><div><strong>Credits</strong><ul class="plain-list">${fallbackCredit.map(row).join('')}</ul></div></div><div class="accounting-grid"><p><strong>Total Debit</strong><br>${formatCurrency(totalDebit)}</p><p><strong>Total Credit</strong><br>${formatCurrency(totalCredit)}</p><p><strong>Status</strong><br>${Math.abs(totalDebit-totalCredit)<0.01?'Balanced':'Out of balance'}</p></div></div>`;
}
...
function renderLoanDetailFields(loan) {
  ...
  const disb = disbursementSummaryFromLoan(loan);
  const disbursementSummaryHtml = `<div class="subcard"><div class="card-header"><div><div class="eyebrow">Disbursement</div><h3>Disbursement Summary</h3></div></div><div class="loan-detail-grid">${[
    ['Gross Principal', formatCurrency(disb.gross)],
    ['Total Deductions', formatCurrency(disb.deductions)],
    ['Net Disbursed', formatCurrency(disb.net)],
    ['Funding Account', disb.fundingAccount],
    ['Disbursement Date', formatDateOnlyDisplay(disb.date) || disb.date || '—'],
    ['Journal Number', disb.journal],
  ].map(([label,value])=>`<div class="loan-detail-stat"><span>${escapeHtml(label)}</span><strong>${escapeHtml(String(value))}</strong></div>`).join('')}</div>${disb.reversed?`<div class="alert warning"><strong>Disbursement reversed</strong><br>Original journal: ${escapeHtml(disb.journal)}<br>Reversal journal: ${escapeHtml(disb.reversalJournal)}<br>Reversal reason: ${escapeHtml(disb.reversalReason)}<br>Reversal date: ${escapeHtml(formatDateOnlyDisplay(disb.reversalDate)||disb.reversalDate||'—')}</div>`:''}${disb.charges.length?`<div class="ledger-table-scroll"><table class="placeholder-table loan-table"><thead><tr><th>Charge</th><th>Gross Amount</th><th>Tax</th><th>Net Charge</th><th>Accounting Treatment</th><th>GL Account</th><th>Status</th></tr></thead><tbody>${disb.charges.map(c=>{ const tax=moneyNumber(c.tax_amount ?? c.taxAmount, c.tax_method==='NO_TAX'?0:(moneyNumber(c.amount)*moneyNumber(c.tax_rate)/100)); return `<tr><td>${escapeHtml(c.charge_type_name)}</td><td>${formatCurrency(c.amount)}</td><td>${formatCurrency(tax)}</td><td>${formatCurrency(Math.max(0,moneyNumber(c.amount)-tax))}</td><td>${escapeHtml(c.accounting_treatment)}</td><td>${escapeHtml(c.gl_account_name || c.gl_account_id || '—')}</td><td>${escapeHtml(disb.reversed?'Reversed':(c.status||'Posted'))}</td></tr>`; }).join('')}</tbody></table></div>`:'<p class="muted">No disbursement deductions recorded.</p>'}</div>`;
  const repairAction = renderLoanRepairAction(loan);
  adminLoanDetailContent.innerHTML = `${warnings}${disbursementSummaryHtml}${renderAccountingSummarySection(loan)}${repairAction ? `<div class="loan-detail-actions">${repairAction}</div>` : ''}<div class="loan-detail-grid">${fields
...
function renderLoanLedger() {
  ...
  const disb = disbursementSummaryFromLoan(loan);
  const proceedsNote = `<p class="muted">Repayment principal: ${formatCurrency(totals.totalPrincipal || disb.gross)}. Net customer proceeds at disbursement: ${formatCurrency(disb.net)}.</p>`;

  if (!entries.length) {
    adminLoanDetailContent.innerHTML = `${scheduleHtml}<div class="ledger-totals-grid">${totalsHtml}</div>${proceedsNote}<p class="muted">No ledger entries found for this loan.</p>`;
    return;
  }
  ...
  adminLoanDetailContent.innerHTML = `${scheduleHtml}<div class="ledger-totals-grid">${totalsHtml}</div>${proceedsNote}
...
async function openApplicationDetail(appSummary, role) {
  ...
  try {
    ...
    const handleDisburse = async () => {
      ...
      modal.className = 'modal-overlay historical-accounting-modal disbursement-deductions-modal';
      ...
      const principal = getGrossLoanPrincipal(app);
      const totalInterest = Number(app.loan_details?.total_interest || app.total_interest || 0);
      let settings = {}, accounts = [], chargeTypes = [], defaultCharges = [];
      try {
        const [settingsRaw, accountsRaw, chargeTypesRaw, defaultsRaw] = await Promise.allSettled([
          api('/admin/accounting/settings'),
          api('/admin/accounting/accounts?active=true'),
          api('/admin/disbursement-charge-types?active=true'),
          api(`/admin/loan-products/${encodeURIComponent(app.product_id || app.loan_product_id || app.loan_details?.product_id || '')}/disbursement-charges?loan_application_id=${encodeURIComponent(appId || '')}`),
        ]);
        settings = settingsRaw.status === 'fulfilled' ? (settingsRaw.value || {}) : {};
        accounts = accountItems(accountsRaw.status === 'fulfilled' ? accountsRaw.value : []).filter(a => (a.active !== false && a.is_active !== false) && String(a.type || a.account_type).toUpperCase() === 'ASSET');
        chargeTypes = normalizeDisbursementCharges(chargeTypesRaw.status === 'fulfilled' ? chargeTypesRaw.value : {});
        defaultCharges = normalizeDisbursementCharges(defaultsRaw.status === 'fulfilled' ? defaultsRaw.value : app);
      } catch (err) { console.error('Failed to load disbursement setup', err); }
      if (!chargeTypes.length) chargeTypes = normalizeDisbursementCharges(app);
      let charges = defaultCharges.length ? defaultCharges : normalizeDisbursementCharges(app);
      ...
      const chargeOptions = () => chargeTypes.map(c=>`<option value="${escapeHtml(c.charge_type_id)}">${escapeHtml(c.charge_type_name)}</option>`).join('');
      modal.innerHTML = `<div class="modal-card wide"><div class="modal-header"><h2>Disburse Loan</h2><button class="icon-button" data-close-disburse>×</button></div><div id="disburse-error"></div><p><strong>Application Number:</strong> ${escapeHtml(appNo)} &nbsp; <strong>Customer Name:</strong> ${escapeHtml(customer)}</p><div class="loan-detail-grid"><div class="loan-detail-stat"><span>Approved/Gross Principal</span><strong>${formatCurrency(principal)}</strong></div><div class="loan-detail-stat"><span>Total Payable</span><strong>${formatCurrency(app.loan_details?.total_payable || app.total_payable || principal)}</strong></div></div><div class="accounting-grid"><label>Disbursement Date *<input id="disburse-date" type="date" value="${todayDateOnly()}"></label><label>Transaction Method *<select id="disburse-method"><option>BANK_TRANSFER</option><option>CASH</option><option>CHEQUE</option><option>OTHER</option></select></label><label>Funding Account *<select id="funding-account"><option value="">Select funding account</option></select></label><label>Reference<input id="disburse-reference" placeholder="Required for bank transfer or cheque"></label><label>Interest Accounting Method<select id="interest-accounting-method" ${methodReadOnly ? 'disabled' : ''}><option value="ACCRUAL_BY_INSTALLMENT">Accrual by installment</option><option value="CASH_BASIS">Cash basis</option></select><small>${methodReadOnly ? 'Inherited from Accounting Settings.' : 'Override allowed by Accounting Settings.'}</small></label><label>Remarks<textarea id="disburse-remarks"></textarea></label></div><div id="historical-disbursement-panel"></div><div class="subcard"><div class="card-header"><div><h3>Disbursement Deductions</h3><p class="muted">Default charges come from the loan product. Backend preview is authoritative.</p></div><button type="button" class="secondary" id="add-deduction">Add Deduction</button></div><div id="deductions-list"></div></div><div id="disbursement-totals"></div><div id="customer-proceeds-warning"></div><button type="button" class="secondary" id="preview-disbursement">Preview Disbursement</button><div id="backend-preview"></div><details id="historical-journal-details" class="subcard hidden"><summary>View details</summary><div id="historical-journal-detail-body"></div></details><div class="modal-actions sticky-modal-footer"><button class="secondary" data-close-disburse>Cancel</button><button id="confirm-disburse" disabled>Confirm Disbursement</button></div></div>`;
      document.body.appendChild(modal);
      const methodEl=modal.querySelector('#disburse-method'), accountEl=modal.querySelector('#funding-account'), refEl=modal.querySelector('#disburse-reference'), dateEl=modal.querySelector('#disburse-date'), confirm=modal.querySelector('#confirm-disburse'), errEl=modal.querySelector('#disburse-error'), interestMethodEl=modal.querySelector('#interest-accounting-method'), historicalPanel=modal.querySelector('#historical-disbursement-panel'), deductionsList=modal.querySelector('#deductions-list'), totalsEl=modal.querySelector('#disbursement-totals'), warningEl=modal.querySelector('#customer-proceeds-warning'), previewBox=modal.querySelector('#backend-preview');
      ...
      const selectedAccountName=()=>{const a=validAccounts().find(x=>String(x.id)===accountEl.value); return a?`${a.code||a.account_code||''} ${a.name||a.account_name||''}`.trim():'Selected Bank/Cash Account';};
      function recalcCharge(c){ if(String(c.calculation_method).includes('PERCENT')) c.amount = principal * moneyNumber(c.rate) / 100; return c; }
      function renderDeductions(){ charges=charges.map(recalcCharge); deductionsList.innerHTML = charges.length ? charges.map((c,i)=>`<div class="deduction-card subcard" data-charge-index="${i}"><div class="accounting-grid"><label>Charge Type *<select data-charge-field="charge_type_id"><option value="">Select charge type</option>${chargeOptions()}</select>${c.mandatory?'<small><span class="badge">Mandatory</span> Default '+formatCurrency(c.amount)+'</small>':''}</label><label>Calculation Method<select data-charge-field="calculation_method"><option>FIXED_AMOUNT</option><option>PERCENTAGE</option></select></label><label>Rate<input data-charge-field="rate" type="number" step="0.01" value="${escapeHtml(c.rate)}"></label><label>Amount *<input data-charge-field="amount" type="number" step="0.01" value="${escapeHtml(c.amount)}"></label><label>Tax Method<select data-charge-field="tax_method"><option>NO_TAX</option><option>INCLUSIVE</option><option>EXCLUSIVE</option></select></label><label>Tax Rate<input data-charge-field="tax_rate" type="number" step="0.01" value="${escapeHtml(c.tax_rate)}"></label><label>Accounting Destination *<select data-charge-field="gl_account_id"><option value="">Select GL account</option>${accounts.map(a=>`<option value="${escapeHtml(a.id)}">${escapeHtml((a.code||a.account_code||'')+' — '+(a.name||a.account_name||''))}</option>`).join('')}</select><small>Posts to: ${escapeHtml(c.gl_account_name || 'Select destination')} (${escapeHtml(c.accounting_treatment)})</small></label><label>Description<textarea data-charge-field="description">${escapeHtml(c.description)}</textarea></label></div><label><input type="checkbox" data-charge-field="waived" ${c.waived?'checked':''} ${c.mandatory?'':' '}> Waive deduction</label><button type="button" class="danger" data-remove-charge ${c.mandatory?'disabled':''}>Remove</button></div>`).join('') : '<p class="muted">No deductions configured. Use Add Deduction to withhold approved charges.</p>';
        deductionsList.querySelectorAll('.deduction-card').forEach(card=>{ const i=Number(card.dataset.chargeIndex); Object.entries(charges[i]).forEach(([k,v])=>{ const el=card.querySelector(`[data-charge-field="${k}"]`); if(el && el.type!=='checkbox') el.value=String(v??''); }); card.querySelectorAll('[data-charge-field]').forEach(el=>el.oninput=()=>{ const key=el.dataset.chargeField; charges[i][key]=el.type==='checkbox'?el.checked:el.value; if(key==='charge_type_id'){ const t=chargeTypes.find(x=>String(x.charge_type_id)===String(el.value)); if(t) charges[i]={...charges[i],...t,uid:charges[i].uid,amount:t.amount||charges[i].amount}; } backendPreview=null; previewBox.innerHTML=''; renderDeductions(); validate(); }); card.querySelector('[data-remove-charge]').onclick=()=>{ charges.splice(i,1); backendPreview=null; renderDeductions(); validate(); }; }); renderTotals(); }
      function totals(){ const total=charges.reduce((a,c)=>a+(c.waived?0:moneyNumber(c.amount)),0); return {total,net:principal-total}; }
      function renderTotals(){ const t=totals(); totalsEl.innerHTML=`<div class="subcard"><div class="accounting-grid"><p><strong>Gross Loan Principal</strong><br>${formatCurrency(principal)}</p><p><strong>Total Deductions</strong><br>${formatCurrency(t.total)}</p><p><strong>Net Amount to Customer</strong><br>${formatCurrency(t.net)}</p></div></div>`; warningEl.innerHTML=`<div class="alert warning">The customer will receive ${formatCurrency(t.net)}. The loan principal remains ${formatCurrency(principal)}.</div>`; previewBox.innerHTML = backendPreview ? previewBox.innerHTML : renderDisbursementJournalPreviewHtml({}, principal, t.net, charges, selectedAccountName()); }
      function renderHistoricalPanel(){ const hist=isHistoricalDate(dateEl.value); if(!hist){historicalPanel.innerHTML=''; modal.querySelector('#historical-journal-details').classList.add('hidden'); return {hist:false,dueCount:0,immediateInterest:0};} const periodStatus=String(settings.accounting_period_status||settings.periodStatus||(settings.accounting_period_locked?'Locked':'Open')); historicalPanel.innerHTML=`<div class="alert warning"><strong>Historical disbursement</strong><div class="accounting-grid"><div><strong>Historical date warning</strong><br>${escapeHtml(formatDateOnlyDisplay(dateEl.value))}</div><div><strong>Charge-income posting date</strong><br>${escapeHtml(formatDateOnlyDisplay(dateEl.value))}</div><div><strong>Bank posting date</strong><br>${escapeHtml(formatDateOnlyDisplay(dateEl.value))}</div><div><strong>Accounting-period status</strong><br>${escapeHtml(periodStatus)}</div></div></div>`; return {hist:true,dueCount:0,immediateInterest:0}; }
      function validationErrors(){ const t=totals(), errors=[]; if(!accountEl.value) errors.push('Select a funding account.'); if(isFutureDateOnly(dateEl.value)) errors.push('Future disbursement dates are not supported.'); charges.filter(c=>!c.waived).forEach((c,i)=>{ if(!c.charge_type_id) errors.push(`Deduction ${i+1}: charge type is required.`); if(moneyNumber(c.amount)<0) errors.push(`${c.charge_type_name||'Deduction'} amount cannot be negative.`); if(!c.gl_account_id) errors.push(`${c.charge_type_name||'Deduction'} destination account is required.`); if(moneyNumber(c.tax_rate)<0 || moneyNumber(c.tax_rate)>100) errors.push(`${c.charge_type_name||'Deduction'} has an invalid tax rate.`); }); if(t.total>principal) errors.push('Total deductions cannot exceed the loan principal.'); if(t.net<=0 && !boolFromBackend(settings.allow_zero_net_disbursement, false)) errors.push('Net disbursed amount must be greater than zero.'); if(['BANK_TRANSFER','CHEQUE'].includes(methodEl.value)&&!refEl.value.trim()) errors.push('Reference is required for bank transfer or cheque.'); return errors; }
      function validate(){ renderHistoricalPanel(); renderTotals(); const errors=validationErrors(); errEl.innerHTML=errors.length?`<div class="alert error"><ul>${errors.map(e=>`<li>${escapeHtml(e)}</li>`).join('')}</ul></div>`:''; confirm.disabled = errors.length>0 || !backendPreview || applicationModalActions.dataset.disbursing==='true'; }
      function payload(){ return { disbursement_date: dateEl.value, funding_account_id: Number(accountEl.value), transaction_method: methodEl.value, reference: refEl.value.trim(), remarks: modal.querySelector('#disburse-remarks').value.trim(), charges: charges.filter(c=>!c.waived).map(c=>({ charge_type_id:Number(c.charge_type_id), amount:moneyNumber(c.amount), tax_method:c.tax_method||'NO_TAX', tax_rate:moneyNumber(c.tax_rate) })) }; }
      async function runPreview(){ const errors=validationErrors(); if(errors.length){ validate(); return; } previewBox.innerHTML='<p>Loading disbursement preview...</p>'; confirm.disabled=true; try{ backendPreview=await api(`/admin/loans/${encodeURIComponent(appId)}/disbursement-preview`,{method:'POST',body:payload()}); const previewCharges=normalizeDisbursementCharges(backendPreview); if(previewCharges.length) charges=previewCharges; const pTotal=moneyNumber(backendPreview.total_deductions ?? backendPreview.totalDeductions, totals().total); const pNet=moneyNumber(backendPreview.net_amount ?? backendPreview.netAmount ?? backendPreview.net_disbursed ?? backendPreview.netDisbursed, principal-pTotal); const warnings=backendPreview.validation_warnings||backendPreview.validationWarnings||backendPreview.warnings||[]; previewBox.innerHTML=`${warnings.length?`<div class="alert warning">${warnings.map(w=>escapeHtml(String(w))).join('<br>')}</div>`:''}${renderDisbursementJournalPreviewHtml(backendPreview, principal, pNet, charges, selectedAccountName())}`; confirm.disabled=false; }catch(e){ backendPreview=null; previewBox.innerHTML=`<div class="alert error">${escapeHtml(e.message||'Disbursement preview failed.')}</div>`; }finally{ validate(); } }
      ...
      modal.querySelector('#add-deduction').onclick=()=>{ charges.push({uid:`new-${Date.now()}`,charge_type_id:'',charge_type_name:'Deduction',calculation_method:'FIXED_AMOUNT',rate:0,amount:0,tax_method:'NO_TAX',tax_rate:0,accounting_treatment:'INCOME',gl_account_id:'',description:''}); backendPreview=null; renderDeductions(); validate(); };
      modal.querySelector('#preview-disbursement').onclick=runPreview;
      modal.querySelectorAll('[data-close-disburse]').forEach(b=>b.onclick=()=>modal.remove()); [methodEl,accountEl,refEl,dateEl,interestMethodEl].forEach(el=>el.addEventListener('input',()=>{ backendPreview=null; validate(); })); methodEl.addEventListener('change',renderAccounts); renderDeductions(); renderAccounts();
      confirm.onclick = async () => { if(confirm.disabled) return; const t=totals(); if(!window.confirm(`Disburse Loan\n\nGross principal:\n${formatCurrency(principal)}\n\nTotal deductions:\n${formatCurrency(t.total)}\n\nNet amount transferred:\n${formatCurrency(t.net)}\n\nCustomer principal obligation:\n${formatCurrency(principal)}`)) return; applicationModalActions.dataset.disbursing='true'; confirm.disabled=true; confirm.textContent='Disbursing...'; errEl.innerHTML=''; try { const res=await api(endpoint('loanApplicationDisburse', { id: appId }), { method:'POST', body: { ...payload(), disbursement_method: methodEl.value, transaction_reference: refEl.value.trim(), accounting_date: dateEl.value, interest_accounting_method: interestMethodEl.value, historical_accrual_option: historicalChoice } }); const journalNo=res.journal_no||res.journalNo||res.journal_number||'created'; const loanNo=res.loan_number||res.loanNo||appNo; modal.querySelector('.modal-card').innerHTML=`<h2>Loan Disbursed</h2><p>Loan Number: ${escapeHtml(loanNo)}</p><p>Journal Number: ${escapeHtml(journalNo)}</p><div class="subcard"><h3>Printable Disbursement Advice</h3><p>Loan Principal ${formatCurrency(principal)}</p>${charges.filter(c=>!c.waived).map(c=>`<p>Less ${escapeHtml(c.charge_type_name)} ${formatCurrency(c.amount)}</p>`).join('')}<p><strong>Net Amount Transferred ${formatCurrency(t.net)}</strong></p><p>Repayment principal ${formatCurrency(principal)}</p><button onclick="window.print()">Print</button></div><button data-close-success>Close</button> <button onclick="showAdminSection('loans')">View Loan</button> <button onclick="showAdminSection('accounting-journals')">View Journal Entry</button>`; modal.querySelector('[data-close-success]').onclick=()=>modal.remove(); setInlineAlert(applicationModalMessage, `Loan ${loanNo} disbursed. Journal ${journalNo} posted.`, 'success'); if (appId) await openApplicationDetail({ ...appSummary, id: appId }, role); if (role === 'admin') await Promise.all([loadAdmin(), loadAdminLoanApplicationsAll(true)]); } catch(err) { console.error('Failed to disburse loan application', err); errEl.innerHTML=`<div class="alert error">${escapeHtml(err.message || 'Failed to disburse loan. Please try again.')}</div>`; } finally { applicationModalActions.dataset.disbursing='false'; confirm.textContent='Confirm Disbursement'; validate(); } };
    };
...
async function renderDisbursementChargeTypesPanel(root) {
  const host = root.querySelector('#disbursement-charge-types-panel');
  if (!host) return;
  host.innerHTML = '<p>Loading disbursement charges...</p>';
  try {
    const [raw, accountsRaw] = await Promise.all([api('/admin/disbursement-charge-types'), api('/admin/accounting/accounts?active=true')]);
    const chargeTypes = normalizeDisbursementCharges(raw);
    const accounts = accountItems(accountsRaw);
    const acctOpts = accounts.map(a=>`<option value="${escapeHtml(a.id)}">${escapeHtml((a.code||a.account_code||'')+' — '+(a.name||a.account_name||''))}</option>`).join('');
    host.innerHTML = `<div class="card-header"><div><h3>Disbursement Charges</h3><p class="muted">Configure charge types deducted from loan disbursements.</p></div><button type="button" id="add-charge-type">Add</button></div><div id="charge-type-message"></div><div class="ledger-table-scroll"><table class="placeholder-table loan-table"><thead><tr><th>Code</th><th>Name</th><th>Calculation Method</th><th>Default Amount/Rate</th><th>Accounting Treatment</th><th>GL Account</th><th>Tax Method</th><th>Active</th><th>Actions</th></tr></thead><tbody>${chargeTypes.map(c=>`<tr><td>${escapeHtml(c.code||c.charge_type_id)}</td><td>${escapeHtml(c.charge_type_name)}</td><td>${escapeHtml(c.calculation_method)}</td><td>${formatCurrency(c.amount)} / ${escapeHtml(c.rate)}%</td><td>${escapeHtml(c.accounting_treatment)}</td><td>${escapeHtml(c.gl_account_name||c.gl_account_id||'—')}</td><td>${escapeHtml(c.tax_method)} ${escapeHtml(c.tax_rate)}%</td><td>${c.active===false?'No':'Yes'}</td><td><button data-edit-charge-type="${escapeHtml(c.charge_type_id)}">Edit</button> <button class="secondary" data-toggle-charge-type="${escapeHtml(c.charge_type_id)}">${c.active===false?'Activate':'Deactivate'}</button></td></tr>`).join('')||'<tr><td colspan="9" class="muted">No charge types found.</td></tr>'}</tbody></table></div>`;
    const msg=host.querySelector('#charge-type-message');
    const openForm=(existing={})=>{ const modal=document.createElement('div'); modal.className='modal-overlay historical-accounting-modal'; modal.innerHTML=`<div class="modal-card wide"><div class="modal-header"><h2>${existing.charge_type_id?'Edit':'Add'} Charge Type</h2><button class="icon-button" data-close>×</button></div><div id="charge-type-error"></div><div class="accounting-grid"><label>Code<input id="ct-code" value="${escapeHtml(existing.code||'')}"></label><label>Charge Name<input id="ct-name" value="${escapeHtml(existing.charge_type_name||'')}"></label><label>Description<textarea id="ct-desc">${escapeHtml(existing.description||'')}</textarea></label><label>Calculation Method<select id="ct-calc"><option>FIXED_AMOUNT</option><option>PERCENTAGE</option></select></label><label>Default Amount<input id="ct-amount" type="number" step="0.01" value="${escapeHtml(existing.amount||0)}"></label><label>Default Rate<input id="ct-rate" type="number" step="0.01" value="${escapeHtml(existing.rate||0)}"></label><label>Accounting Treatment<select id="ct-treatment"><option>INCOME</option><option>PAYABLE</option><option>TAX</option><option>EXPENSE_RECOVERY</option></select></label><label>Destination GL Account<select id="ct-gl"><option value="">Select GL account</option>${acctOpts}</select></label><label>Tax Method<select id="ct-tax-method"><option>NO_TAX</option><option>INCLUSIVE</option><option>EXCLUSIVE</option></select></label><label>Tax Rate<input id="ct-tax-rate" type="number" step="0.01" value="${escapeHtml(existing.tax_rate||0)}"></label><label><input id="ct-refundable" type="checkbox" ${existing.refundable?'checked':''}> Refundable</label><label><input id="ct-deduct" type="checkbox" ${existing.deduct_from_disbursement!==false?'checked':''}> Deduct from Disbursement</label><label><input id="ct-active" type="checkbox" ${existing.active===false?'':'checked'}> Active</label></div><div class="modal-actions sticky-modal-footer"><button class="secondary" data-close>Cancel</button><button id="save-charge-type">Save</button></div></div>`; document.body.appendChild(modal); modal.querySelector('#ct-calc').value=existing.calculation_method||'FIXED_AMOUNT'; modal.querySelector('#ct-treatment').value=existing.accounting_treatment||'INCOME'; modal.querySelector('#ct-gl').value=existing.gl_account_id||''; modal.querySelector('#ct-tax-method').value=existing.tax_method||'NO_TAX'; modal.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>modal.remove()); modal.querySelector('#save-charge-type').onclick=async()=>{ const body={code:modal.querySelector('#ct-code').value.trim(), name:modal.querySelector('#ct-name').value.trim(), description:modal.querySelector('#ct-desc').value.trim(), calculation_method:modal.querySelector('#ct-calc').value, default_amount:moneyNumber(modal.querySelector('#ct-amount').value), default_rate:moneyNumber(modal.querySelector('#ct-rate').value), accounting_treatment:modal.querySelector('#ct-treatment').value, destination_gl_account_id:Number(modal.querySelector('#ct-gl').value)||undefined, tax_method:modal.querySelector('#ct-tax-method').value, tax_rate:moneyNumber(modal.querySelector('#ct-tax-rate').value), refundable:modal.querySelector('#ct-refundable').checked, deduct_from_disbursement:modal.querySelector('#ct-deduct').checked, active:modal.querySelector('#ct-active').checked}; try{ await api(existing.charge_type_id?`/admin/disbursement-charge-types/${encodeURIComponent(existing.charge_type_id)}`:'/admin/disbursement-charge-types',{method:existing.charge_type_id?'PUT':'POST',body}); modal.remove(); renderDisbursementChargeTypesPanel(root); }catch(e){ modal.querySelector('#charge-type-error').innerHTML=`<div class="alert error">${escapeHtml(e.message||'Failed to save charge type.')}</div>`; } }; };
    host.querySelector('#add-charge-type').onclick=()=>openForm({});
    host.querySelectorAll('[data-edit-charge-type]').forEach(b=>b.onclick=()=>openForm(chargeTypes.find(c=>String(c.charge_type_id)===String(b.dataset.editChargeType))||{}));
    host.querySelectorAll('[data-toggle-charge-type]').forEach(b=>b.onclick=async()=>{ try{ await api(`/admin/disbursement-charge-types/${encodeURIComponent(b.dataset.toggleChargeType)}/toggle`,{method:'POST',body:{}}); renderDisbursementChargeTypesPanel(root); }catch(e){ msg.innerHTML=`<div class="alert error">${escapeHtml(e.message)}</div>`; } });
  } catch (e) { host.innerHTML = `<div class="alert error">${escapeHtml(e.message||'Failed to load disbursement charge types.')}</div>`; }
}
...
async function accountingLoadSettings(){
  ...
  renderDisbursementChargeTypesPanel(root);
...
async function loadFinancialReports(){const root=document.querySelector('#accounting-reports-root'); if(!root)return; reportShell(root,'Financial Reports',`<label>Date From<input name="date_from" type="date"></label><label>Date To<input name="date_to" type="date"></label><label>As of Date<input name="as_of_date" type="date" value="${new Date().toISOString().slice(0,10)}"></label>`,'Loading...'); const form=root.querySelector('form'); async function run(e){e&&e.preventDefault(); const box=root.querySelector('.report-results'); box.innerHTML='Loading...'; try{const d=await api('/admin/accounting/reports/summary?'+reportQuery(form)); const cards=['total_assets','total_liabilities','total_equity','total_income','total_expenses','net_profit_loss','trial_balance_difference','financial_position_difference'].map(k=>`<div class="metric"><div class="metric-label">${k.replaceAll('_',' ')}</div><div class="metric-value">${reportCurrency(d[k])}</div></div>`).join(''); box.innerHTML=`${reportWarnings(d)}<div class="accounting-grid">${cards}</div><div class="subcard"><h3>Status</h3><p>Trial Balance: ${d.trial_balance_balanced?'Balanced':'Out of Balance'}</p><p>Statement of Financial Position: ${d.financial_position_balanced?'Balanced':'Out of Balance'}</p><p>Unclassified Accounts: ${escapeHtml(d.unclassified_accounts??0)}</p><p>${d.incomplete_accounting_history?'Incomplete Accounting History warning':'Accounting history OK'}</p></div><div class="subcard"><h3>Disbursement Register</h3><p class="muted">Columns: Date, Loan, Customer, Gross Principal, Charges, Net Disbursed, Funding Account, Journal.</p><button class="secondary" id="run-disbursement-register">Load Disbursement Register</button><div id="disbursement-register-results"></div></div><div class="subcard"><h3>Disbursement Charges Register</h3><p class="muted">Columns: Date, Loan, Customer, Charge Type, Amount, Tax, GL Account, Status.</p><button class="secondary" id="run-disbursement-charges-register">Load Charges Register</button><div id="disbursement-charges-register-results"></div></div><div class="action-row"><button data-accounting-section="accounting-trial-balance">Open Trial Balance</button><button data-accounting-section="accounting-income-statement">Open Income Statement</button><button data-accounting-section="accounting-financial-position">Open Statement of Financial Position</button><button class="secondary" data-accounting-section="accounting-reconciliation">Open Reconciliation</button></div>`; const reg=box.querySelector('#run-disbursement-register'), chg=box.querySelector('#run-disbursement-charges-register'); reg.onclick=async()=>{const out=box.querySelector('#disbursement-register-results'); out.innerHTML='Loading...'; try{const r=await api('/admin/reports/disbursement-register?'+reportQuery(form)); const rows=reportItems(r.items||r.data||r); out.innerHTML=reportTable(['Date','Loan','Customer','Gross Principal','Charges','Net Disbursed','Funding Account','Journal'],rows.map(x=>`<tr><td>${escapeHtml(x.date||x.disbursement_date||'')}</td><td>${escapeHtml(x.loan_number||x.loan||'')}</td><td>${escapeHtml(x.customer_name||x.customer||'')}</td><td class="money">${reportCurrency(x.gross_principal)}</td><td class="money">${reportCurrency(x.charges||x.total_deductions)}</td><td class="money">${reportCurrency(x.net_disbursed)}</td><td>${escapeHtml(x.funding_account||'')}</td><td>${escapeHtml(x.journal_number||x.journal||'')}</td></tr>`));}catch(e){out.innerHTML=`<div class="alert error">${escapeHtml(e.message)}</div>`;}}; chg.onclick=async()=>{const out=box.querySelector('#disbursement-charges-register-results'); out.innerHTML='Loading...'; try{const r=await api('/admin/reports/disbursement-charges-register?'+reportQuery(form)); const rows=reportItems(r.items||r.data||r); out.innerHTML=reportTable(['Date','Loan','Customer','Charge Type','Amount','Tax','GL Account','Status'],rows.map(x=>`<tr><td>${escapeHtml(x.date||x.disbursement_date||'')}</td><td>${escapeHtml(x.loan_number||x.loan||'')}</td><td>${escapeHtml(x.customer_name||x.customer||'')}</td><td>${escapeHtml(x.charge_type||x.charge_type_name||'')}</td><td class="money">${reportCurrency(x.amount)}</td><td class="money">${reportCurrency(x.tax)}</td><td>${escapeHtml(x.gl_account||x.gl_account_name||'')}</td><td>${escapeHtml(x.status||'')}</td></tr>`));}catch(e){out.innerHTML=`<div class="alert error">${escapeHtml(e.message)}</div>`;}};}catch(err){box.innerHTML=`<div class="alert error">${escapeHtml(err.message)}</div>`;}} form.onsubmit=run; await run();}
