...
function resetAccountEditorState(){ document.querySelectorAll('.account-editor-modal').forEach(m=>m.remove()); const msg=document.querySelector('#account-editor-message'); if(msg)msg.innerHTML=''; window.currentEditingAccount=null; }
function accountOptions(selectedId,excludeId){ return (accountingState.accounts||[]).filter(a=>String(a.id)!==String(excludeId||'')).map(a=>`<option value="${escapeHtml(a.id)}" ${String(a.id)===String(selectedId||'')?'selected':''}>${escapeHtml(accountLabel(a))}</option>`).join(''); }
...
document.addEventListener('click', async e=>{ const edit=e.target.closest('.edit-account-btn'); if(edit){ e.preventDefault(); try{ await openAccountEditor(Number(edit.dataset.accountId)); }catch(err){ const msg=document.querySelector('#account-editor-message'); if(msg)msg.innerHTML=`<div class="alert error">${escapeHtml(accountApiError(err,'Failed to open account editor.'))}</div>`; else alert(accountApiError(err,'Failed to open account editor.')); } return; } const add=e.target.closest('[data-account-form]'); if(!add)return; if(add.classList.contains('edit-account-btn'))return; if(!(collectionState.collectors||[]).length || !(accountingState.accounts||[]).length) await collectionBootstrapData(); openCollectionAccountForm({}); });
