...
function renderBankReconciliationWorkspace(root,r){const rows=bankReconciliationState.transactions,status=String(r.status||'DRAFT').toUpperCase(),editable=status!=='COMPLETED',difference=Number(brField(r,['difference'],0))||0,matchedCount=persistedBankMatchCount(r,rows),hasTransactions=rows.length>0,canComplete=!bankReconciliationState.matchPending&&!bankReconciliationState.selectedJournalLineIds.size&&difference===0&&(!hasTransactions||matchedCount>0),body=bankReconciliationState.transactionsLoading?'<tr><td colspan="9">Loading bank GL transactions…</td></tr>':bankReconciliationState.transactionsError?'<tr><td colspan="9"><div class="alert error">Unable to load bank GL transactions. <button type="button" data-br-retry-lines>Retry</button></div></td></tr>':rows.length?rows.map(line=>{const source=line.raw||line,currentNo=reconciliationDetail(source).number;return `<tr><td>${editable&&!line.isReconciled&&line.id!==undefined&&line.id!==null?`<input type="checkbox" data-br-line="${escapeHtml(line.id)}" aria-label="Select bank GL line" ${bankReconciliationState.selectedJournalLineIds.has(String(line.id))?'checked':''}>`:'—'}</td><td>${escapeHtml(formatDateOnlyDisplay(line.date)||'—')}</td><td>${escapeHtml(line.journalNumber)}</td><td>${escapeHtml(line.description)}</td><td>${escapeHtml(line.reference)}</td><td>${moneyCell(line.debit)}</td><td>${moneyCell(line.credit)}</td><td>${moneyCell(line.runningBalance)}</td><td>${line.isReconciled?`${renderGlReconciliation(source)}${currentNo!=='—'?`<br><small>${escapeHtml(currentNo)}</small>`:''}`:'<span class="reconciliation-status unreconciled">UNRECONCILED</span>'}${editable&&line.isReconciled&&String(currentNo)===String(reconciliationNumber(r))?`<br><button class="link-button" data-br-unmatch="${escapeHtml(line.id)}">Remove Match</button>`:''}</td></tr>`;}).join(''):'<tr><td colspan="9">No bank GL transactions found for this period.</td></tr>';
 ...
 root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Bank Reconciliation</div><h2>${escapeHtml(reconciliationNumber(r))}</h2><p>${escapeHtml(brField(r,['bank_account_label','bank_account_name','account_name'],'—'))} · ${escapeHtml(formatDateOnlyDisplay(brField(r,['statement_date_from','date_from'])))} – ${escapeHtml(formatDateOnlyDisplay(brField(r,['statement_date_to','date_to'])))} · ${brStatus(status)}</p></div><div data-no-print><button class="secondary" data-br-list>Back</button> <button class="secondary" data-br-print-current>Print Reconciliation</button> <button class="secondary" data-br-csv>Export CSV</button></div></div><div id="bank-reconciliation-message">${completionWarning}</div><div class="reconciliation-print-meta"><p>Prepared By: ${escapeHtml(brField(r,['prepared_by_name','prepared_by'],'—'))} · Completed By: ${escapeHtml(brField(r,['completed_by_name','completed_by'],'—'))}</p></div><div class="summary-grid"><div class="summary-card"><span>Matched transaction count</span><strong>${matchedCount}</strong></div>${reconciliationTotals(r).map(([l,v])=>`<div class="summary-card"><span>${l}</span><strong class="${l==='Difference'&&Number(v)!==0?'difference-error':''}">${formatCurrency(v)}</strong></div>`).join('')}</div><div class="table-scroll"><table><thead><tr>${['Select','Date','Journal No.','Description','Reference','Debit','Credit','Running Balance','Reconciled'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${body}</tbody></table></div><div class="sticky-actions" data-no-print>${editable?`<button id="mark-bank-reconciled" ${!bankReconciliationState.selectedJournalLineIds.size||bankReconciliationState.matchPending?'disabled':''}>${bankReconciliationState.matchPending?'Saving…':'Mark Reconciled'}</button><button id="complete-bank-reconciliation" ${canComplete?'':'disabled'}>Complete Reconciliation</button>`:'<span class="muted">Completed reconciliations are read-only.</span>'}</div>`;
 root.querySelector('[data-br-list]').onclick=accountingLoadBankReconciliation;root.querySelector('[data-br-print-current]').onclick=()=>window.print();root.querySelector('[data-br-csv]').onclick=()=>exportBankReconciliationCsv(r,rows);root.querySelector('[data-br-retry-lines]')?.addEventListener('click',()=>openBankReconciliation(reconciliationId(r)));root.querySelectorAll('[data-br-line]').forEach(cb=>cb.onchange=()=>{cb.checked?bankReconciliationState.selectedJournalLineIds.add(cb.dataset.brLine):bankReconciliationState.selectedJournalLineIds.delete(cb.dataset.brLine);console.debug('Selected bank journal line IDs',[...bankReconciliationState.selectedJournalLineIds]);const mark=root.querySelector('#mark-bank-reconciled');if(mark)mark.disabled=!bankReconciliationState.selectedJournalLineIds.size||bankReconciliationState.matchPending;const complete=root.querySelector('#complete-bank-reconciliation');if(complete)complete.disabled=true;});const mark=root.querySelector('#mark-bank-reconciled');if(mark)mark.onclick=()=>markBankLinesReconciled();const complete=root.querySelector('#complete-bank-reconciliation');if(complete)complete.onclick=()=>completeBankReconciliation(r);}
async function markBankLinesReconciled(){const r=bankReconciliationState.current,root=document.querySelector('#accounting-bank-reconciliation-root'),lineIds=[...bankReconciliationState.selectedJournalLineIds];if(!r||String(r.status||'').toUpperCase()==='COMPLETED'||!lineIds.length||bankReconciliationState.matchPending)return;bankReconciliationState.matchPending=true;renderBankReconciliationWorkspace(root,r);try{await api(`${bankReconciliationApi}/${encodeURIComponent(reconciliationId(r))}/lines`,{method:'POST',body:{journal_line_ids:lineIds,reconciled_date:brField(r,['statement_date_to','date_to']),statement_reference:brField(r,['statement_reference'],undefined)}});bankReconciliationState.matchPending=false;bankReconciliationState.selectedJournalLineIds.clear();window.dispatchEvent(new CustomEvent('bank-reconciliation-updated',{detail:{journalLineIds:lineIds}}));await openBankReconciliation(reconciliationId(r));}catch(e){bankReconciliationState.matchPending=false;renderBankReconciliationWorkspace(root,r);root.querySelector('#bank-reconciliation-message').innerHTML=`<div class="alert error">${escapeHtml(requestErrorMessage(e))}</div>`;}}
