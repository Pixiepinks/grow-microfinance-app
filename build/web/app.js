...
function investorPageShell(root){ clearInvestorPageChrome(); const contentRoot=root.closest('.admin-content')||document.querySelector('#admin-content')||document.querySelector('.admin-content')||root; if(contentRoot){ contentRoot.style.flex='1 1 0%'; contentRoot.style.minWidth='0'; contentRoot.style.width='100%'; contentRoot.style.maxWidth='none'; } root.replaceChildren(); root.innerHTML=investorWrapRouteHtml('investors','<div class="page-card investor-page investors-dashboard-v2" data-ui-version="investors-v2"><div id="investor-message"></div><div id="investor-page-body"><p class="investor-loading">Loading investors...</p></div></div>','<button type="button" id="add-investor-btn" class="investor-primary-action" data-add-investor>Add Investor</button>'); console.log('Investor page root', contentRoot||root); return root.querySelector('#investor-page-body'); }
...
function updateInvestorFundingActiveNav(){ const path=window.location.pathname; document.querySelectorAll('[data-investor-funding-nav] .admin-menu-item').forEach(link=>{ const active=link.getAttribute('href')===path; link.classList.toggle('active', active); link.setAttribute('aria-current', active?'page':'false'); }); }
function cleanupInvestorFundingRoute(){ clearInvestorPageChrome(); document.querySelectorAll('.admin-content,#admin-panel,.admin-main').forEach(el=>{ el.classList.remove('split-view','customer-detail-pane','has-detail-pane'); el.style.gridTemplateColumns=''; el.style.marginLeft=''; el.style.left=''; el.style.maxWidth=''; }); }
function navigateToAdminRoute(path){ if(window.location.pathname!==path) history.pushState({adminRoute:path},'',path); renderAdminRoute(path); }
