...
async function markBankLinesReconciled(){const r=bankReconciliationState.current,root=document.querySelector('#accounting-bank-reconciliation-root');if(!r||String(r.status||'').toUpperCase()==='COMPLETED'||bankReconciliationState.matchPending)return;const selectedIds=Array.from(root.querySelectorAll('.bank-reconciliation-line-checkbox:checked')).map(checkbox=>checkbox.dataset.journalLineId||checkbox.value).filter(value=>value!==null&&value!==undefined&&value!=='').map(value=>Number(value)).filter(value=>Number.isInteger(value)&&value>0),journalLineIds=[...new Set(selectedIds)];if(!journalLineIds.length){root.querySelector('#bank-reconciliation-message').innerHTML='<div class="alert error">Select at least one bank GL transaction before marking it reconciled.</div>';return;}const id=reconciliationId(r);console.debug('Bank reconciliation selected IDs',{reconciliationId:id,journalLineIds,count:journalLineIds.length});bankReconciliationState.matchPending=true;renderBankReconciliationWorkspace(root,r);try{await api(`${bankReconciliationApi}/${encodeURIComponent(id)}/lines`,{method:'POST',body:{journal_line_ids:journalLineIds,reconciled_date:brField(r,['statement_date_to','date_to']),statement_reference:brField(r,['statement_reference'],null)}});bankReconciliationState.matchPending=false;window.dispatchEvent(new CustomEvent('bank-reconciliation-updated',{detail:{journalLineIds}}));await openBankReconciliation(id);}catch(e){bankReconciliationState.matchPending=false;renderBankReconciliationWorkspace(root,r);root.querySelector('#bank-reconciliation-message').innerHTML=`<div class="alert error">${escapeHtml(requestErrorMessage(e))}</div>`;}}
