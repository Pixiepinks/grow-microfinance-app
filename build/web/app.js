...
document.addEventListener('click', async e=>{ const edit=e.target.closest('.edit-account-btn'); if(edit){ e.preventDefault(); await openAccountEditor(Number(edit.dataset.accountId)); return; } const add=e.target.closest('[data-account-form]'); if(!add)return; if(add.classList.contains('edit-account-btn'))return; if(!(collectionState.collectors||[]).length || !(accountingState.accounts||[]).length) await collectionBootstrapData(); openCollectionAccountForm({}); });
