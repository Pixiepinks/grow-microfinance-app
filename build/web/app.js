...
const investorFundingRoutes = {
  investors: '/admin/investor-funding/investors',
...
const investorFundingState = { investors: [], agreements: [], accounts: [], settings: {}, lastFundingResult: null };
...
const investorIsBank = a => ['BANK','CASH'].includes(String(a?.account_subtype||a?.subtype||'').toUpperCase()) || String(a?.account_type||a?.type||'').toUpperCase()==='ASSET';
const investorToday = () => todayDateOnly();
function investorApiError(e){ if(e?.status===401) return 'Your session expired or is unauthorized. Please sign in again.'; if(e?.status===403) return 'You do not have permission to perform this investor funding action.'; if(e?.status===404) return 'The investor funding record was not found.'; if(e?.status===422) return e.message || 'The investor funding request contains validation errors.'; if(e?.status===500) return 'The server could not process the investor funding request.'; if(e?.name==='AbortError') return 'Investor funding request timed out. Please retry.'; return e?.message || 'Investor funding request failed.'; }
...
async function investorBootstrap(){ const [inv, agr, acc, settings] = await Promise.allSettled([api(investorFundingRoutes.investors), api(investorFundingRoutes.agreements), api('/admin/accounting/accounts'), api(investorFundingRoutes.settings)]); investorFundingState.investors=investorItems(inv.value); investorFundingState.agreements=investorItems(agr.value); investorFundingState.accounts=investorItems(acc.value); investorFundingState.settings=settings.value||{}; }
function investorConfigWarning(){ const s=investorFundingState.settings||{}; const missing=[['Investor Borrowings Control Account','investor_borrowings_control_account_id'],['Investor Interest Expense Account','investor_interest_expense_account_id'],['Accrued Investor Interest Payable Account','accrued_investor_interest_payable_account_id'],['Withholding Tax Payable Account','withholding_tax_payable_account_id'],['Default Funding Bank Account','default_funding_bank_account_id']].filter(([,k])=>!s[k]); return missing.length?`<div class="alert warning"><strong>Investor funding accounting configuration is incomplete.</strong><br>Missing: ${missing.map(([l])=>escapeHtml(l)).join(', ')}<br><button data-accounting-section="accounting-settings">Configure Investor Accounts</button></div>`:''; }
...
async function loadInvestorsPage(){ const root=document.querySelector('#investor-funding-investors-root'); if(!root)return; root.innerHTML='<h2>Investors</h2><p>Loading...</p>'; try{ await investorBootstrap(); const rows=investorFundingState.investors; root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Investor Funding</div><h2>Investors</h2></div><button data-add-investor>Add Investor</button></div>${investorConfigWarning()}<div id="investor-message"></div><div class="table-scroll collection-responsive-table"><table><thead><tr>${['Investor Number','Investor Name','Type','NIC / Registration Number','Mobile','Active Agreements','Principal Balance','Accrued Interest','Status','Actions'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr><td>${escapeHtml(investorField(r,'investor_number','number','id'))}</td><td>${escapeHtml(investorField(r,'investor_name','name','full_name','company_name'))}</td><td><span class="badge">${escapeHtml(investorField(r,'type','investor_type'))}</span></td><td>${escapeHtml(investorField(r,'nic','nic_number','registration_number','company_registration_number'))}</td><td>${escapeHtml(investorField(r,'mobile','phone'))}</td><td>${escapeHtml(investorField(r,'active_agreements','activeAgreements')||0)}</td><td>${investorMoney(investorField(r,'principal_balance','current_principal'))}</td><td>${investorMoney(investorField(r,'accrued_interest','accrued_unpaid_interest'))}</td><td><span class="badge">${escapeHtml(investorField(r,'status')||'ACTIVE')}</span></td><td><button data-investor-view="${escapeHtml(r.id)}">View</button> <button data-investor-edit="${escapeHtml(r.id)}">Edit</button> <button data-new-agreement="${escapeHtml(r.id)}">New Agreement</button> <button data-investor-status="${escapeHtml(r.id)}" data-status="ACTIVE">Activate</button> <button data-investor-status="${escapeHtml(r.id)}" data-status="INACTIVE">Deactivate</button></td></tr>`).join('')||'<tr><td colspan="10">No investors found.</td></tr>'}</tbody></table></div>`; }catch(e){root.innerHTML=`<div class="alert error">${escapeHtml(investorApiError(e))}</div>`;} }
function openInvestorForm(existing={}){ const modal=document.createElement('div'); modal.className='modal-overlay historical-accounting-modal'; modal.innerHTML=`<div class="modal-card wide"><div class="modal-header"><h2>${existing.id?'Edit':'Add'} Investor</h2><button class="icon-button" data-close>×</button></div><div id="investor-form-msg"></div><div class="accounting-grid"><label>Investor type<select id="if-type"><option value="INDIVIDUAL">Individual</option><option value="COMPANY">Company</option></select></label><label data-individual>Full name<input id="if-full" value="${escapeHtml(existing.full_name||existing.name||'')}"></label><label data-company>Company name<input id="if-company" value="${escapeHtml(existing.company_name||'')}"></label><label data-individual>NIC<input id="if-nic" value="${escapeHtml(existing.nic||existing.nic_number||'')}"></label><label data-company>Company registration number<input id="if-reg" value="${escapeHtml(existing.company_registration_number||'')}"></label><label>Tax identification number<input id="if-tax" value="${escapeHtml(existing.tax_identification_number||'')}"></label><label>Mobile<input id="if-mobile" value="${escapeHtml(existing.mobile||'')}"></label><label>Email<input id="if-email" type="email" value="${escapeHtml(existing.email||'')}"></label><label>Address<textarea id="if-address">${escapeHtml(existing.address||'')}</textarea></label><label>Bank name<input id="if-bank" value="${escapeHtml(existing.bank_name||'')}"></label><label>Bank branch<input id="if-branch" value="${escapeHtml(existing.bank_branch||'')}"></label><label>Bank account name<input id="if-acct-name" value="${escapeHtml(existing.bank_account_name||'')}"></label><label>Bank account number<input id="if-acct-no" value="${escapeHtml(existing.bank_account_number||'')}"></label><label>Notes<textarea id="if-notes">${escapeHtml(existing.notes||'')}</textarea></label><label>Status<select id="if-status"><option>ACTIVE</option><option>INACTIVE</option></select></label></div><div class="modal-actions sticky-modal-footer"><button class="secondary" data-close>Cancel</button><button id="save-investor">Save Investor</button></div></div>`; document.body.appendChild(modal); const type=modal.querySelector('#if-type'); type.value=existing.investor_type||existing.type||'INDIVIDUAL'; const sync=()=>{ modal.querySelectorAll('[data-individual]').forEach(e=>e.style.display=type.value==='INDIVIDUAL'?'':'none'); modal.querySelectorAll('[data-company]').forEach(e=>e.style.display=type.value==='COMPANY'?'':'none');}; type.onchange=sync; sync(); modal.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>modal.remove()); modal.querySelector('#save-investor').onclick=async()=>{ const body={investor_type:type.value,full_name:modal.querySelector('#if-full').value,company_name:modal.querySelector('#if-company').value,nic:modal.querySelector('#if-nic').value,company_registration_number:modal.querySelector('#if-reg').value,tax_identification_number:modal.querySelector('#if-tax').value,mobile:modal.querySelector('#if-mobile').value,email:modal.querySelector('#if-email').value,address:modal.querySelector('#if-address').value,bank_name:modal.querySelector('#if-bank').value,bank_branch:modal.querySelector('#if-branch').value,bank_account_name:modal.querySelector('#if-acct-name').value,bank_account_number:modal.querySelector('#if-acct-no').value,notes:modal.querySelector('#if-notes').value,status:modal.querySelector('#if-status').value}; try{ await api(existing.id?`${investorFundingRoutes.investors}/${encodeURIComponent(existing.id)}`:investorFundingRoutes.investors,{method:existing.id?'PATCH':'POST',body}); modal.remove(); loadInvestorsPage(); }catch(e){ modal.querySelector('#investor-form-msg').innerHTML=`<div class="alert error">${escapeHtml(investorApiError(e))}</div>`; } }; }
function agreementForm(existing={}){ const acc=investorFundingState.accounts; const opts=investorOptions(investorFundingState.investors, r=>investorField(r,'investor_name','name','full_name','company_name')); const acctOpts=investorOptions(acc, investorAccountLabel); const modal=document.createElement('div'); modal.className='modal-overlay historical-accounting-modal'; modal.innerHTML=`<div class="modal-card wide"><div class="modal-header"><h2>${existing.id?'Edit':'Create'} Funding Agreement</h2><button class="icon-button" data-close>×</button></div><div id="agreement-msg"></div><div class="accounting-grid"><label>Investor<select id="ag-investor">${opts}</select></label><label>Agreement name<input id="ag-name" value="${escapeHtml(existing.agreement_name||'')}"></label><label>Agreement date<input id="ag-date" type="date" value="${escapeHtml(String(existing.agreement_date||investorToday()).slice(0,10))}"></label><label>Start date<input id="ag-start" type="date" value="${escapeHtml(String(existing.start_date||investorToday()).slice(0,10))}"></label><label>Maturity date<input id="ag-maturity" type="date" value="${escapeHtml(String(existing.maturity_date||'').slice(0,10))}"></label><label>Original expected principal<input id="ag-principal" type="number" step="0.01" value="${escapeHtml(existing.original_expected_principal||existing.original_principal||'')}"></label><label>Interest rate<input id="ag-rate" type="number" step="0.01" value="${escapeHtml(existing.interest_rate||2)}"><small id="ag-rate-display"></small></label><label>Interest rate period<select id="ag-period"><option value="MONTHLY">Monthly</option><option value="ANNUAL">Annual</option></select></label><label>Interest calculation method<select id="ag-method"><option value="MONTHLY_AVERAGE_DAILY_BALANCE">Monthly Average Daily Balance</option><option value="DAILY_BALANCE">Daily Balance</option><option value="FIXED_MONTHLY">Fixed Monthly</option></select><small>Interest is calculated using the selected balance method and posted at month-end.</small></label><label>Interest payment frequency<select id="ag-frequency"><option>MONTHLY</option><option>QUARTERLY</option><option>AT_MATURITY</option></select></label><label>Compounding method<select id="ag-compounding"><option>NONE</option><option>CAPITALIZE_MONTHLY</option></select></label><label>Day-count basis<select id="ag-day"><option>ACTUAL_365</option><option>ACTUAL_360</option><option>30_360</option></select></label><label>Interest payment method<select id="ag-pay-method"><option>BANK_TRANSFER</option><option>CAPITALIZE</option></select></label><label>Funding bank account<select id="ag-bank">${acctOpts}</select></label><label>Investor liability account<select id="ag-liability">${acctOpts}</select></label><label>Investor interest expense account<select id="ag-expense">${acctOpts}</select></label><label>Accrued interest payable account<select id="ag-payable">${acctOpts}</select></label><label>Withholding tax account<select id="ag-tax-account">${acctOpts}</select></label><label>Withholding tax rate<input id="ag-tax" type="number" step="0.01" value="${escapeHtml(existing.withholding_tax_rate||0)}"></label><label>Allow additional funding<select id="ag-add"><option value="true">Yes</option><option value="false">No</option></select></label><label>Allow partial repayment<select id="ag-partial"><option value="true">Yes</option><option value="false">No</option></select></label><label>Auto-accrual enabled<select id="ag-auto"><option value="true">Yes</option><option value="false">No</option></select></label><label>Auto-capitalize interest<select id="ag-cap"><option value="false">No</option><option value="true">Yes</option></select></label><label>Status<select id="ag-status"><option>ACTIVE</option><option>DRAFT</option><option>CLOSED</option></select></label></div><div class="modal-actions sticky-modal-footer"><button class="secondary" data-close>Cancel</button><button id="save-agreement">Save Agreement</button></div></div>`; document.body.appendChild(modal); modal.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>modal.remove()); const rate=modal.querySelector('#ag-rate'), period=modal.querySelector('#ag-period'), display=modal.querySelector('#ag-rate-display'); const sync=()=>display.textContent=investorRate(rate.value,period.value); rate.oninput=period.onchange=sync; sync(); modal.querySelector('#save-agreement').onclick=async()=>{ const body={investor_id:modal.querySelector('#ag-investor').value,agreement_name:modal.querySelector('#ag-name').value,agreement_date:modal.querySelector('#ag-date').value,start_date:modal.querySelector('#ag-start').value,maturity_date:modal.querySelector('#ag-maturity').value||null,original_expected_principal:Number(modal.querySelector('#ag-principal').value||0),interest_rate:Number(rate.value||0),interest_rate_period:period.value,interest_calculation_method:modal.querySelector('#ag-method').value,interest_payment_frequency:modal.querySelector('#ag-frequency').value,compounding_method:modal.querySelector('#ag-compounding').value,day_count_basis:modal.querySelector('#ag-day').value,interest_payment_method:modal.querySelector('#ag-pay-method').value,funding_bank_account_id:modal.querySelector('#ag-bank').value,investor_liability_account_id:modal.querySelector('#ag-liability').value,investor_interest_expense_account_id:modal.querySelector('#ag-expense').value,accrued_interest_payable_account_id:modal.querySelector('#ag-payable').value,withholding_tax_account_id:modal.querySelector('#ag-tax-account').value,withholding_tax_rate:Number(modal.querySelector('#ag-tax').value||0),allow_additional_funding:modal.querySelector('#ag-add').value==='true',allow_partial_repayment:modal.querySelector('#ag-partial').value==='true',auto_accrual_enabled:modal.querySelector('#ag-auto').value==='true',auto_capitalize_interest:modal.querySelector('#ag-cap').value==='true',status:modal.querySelector('#ag-status').value}; try{ await api(existing.id?`${investorFundingRoutes.agreements}/${encodeURIComponent(existing.id)}`:investorFundingRoutes.agreements,{method:existing.id?'PATCH':'POST',body}); modal.remove(); loadAgreementsPage(); }catch(e){ modal.querySelector('#agreement-msg').innerHTML=`<div class="alert error">${escapeHtml(investorApiError(e))}</div>`; } }; }
async function loadAgreementsPage(){ const root=document.querySelector('#investor-funding-agreements-root'); if(!root)return; root.innerHTML='<h2>Funding Agreements</h2><p>Loading...</p>'; try{ await investorBootstrap(); const rows=investorFundingState.agreements; root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Investor Funding</div><h2>Funding Agreements</h2></div><button data-add-agreement>Create Agreement</button></div>${investorConfigWarning()}<div class="table-scroll collection-responsive-table"><table><thead><tr>${['Agreement Number','Investor','Start Date','Maturity Date','Original Principal','Current Principal','Interest Rate','Rate Period','Calculation Method','Accrued Interest','Status','Actions'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.map(a=>`<tr><td>${escapeHtml(investorField(a,'agreement_number','number','id'))}</td><td>${escapeHtml(investorField(a,'investor_name','investor'))}</td><td>${investorDate(a.start_date)}</td><td>${investorDate(a.maturity_date)}</td><td>${investorMoney(investorField(a,'original_principal','original_expected_principal'))}</td><td>${investorMoney(investorField(a,'current_principal','principal_balance'))}</td><td>${escapeHtml(investorRate(investorField(a,'interest_rate'), investorField(a,'interest_rate_period','rate_period')))}</td><td>${escapeHtml(investorField(a,'interest_rate_period','rate_period')||'MONTHLY')}</td><td>${escapeHtml(investorField(a,'interest_calculation_method','calculation_method')||'Monthly Average Daily Balance')}</td><td>${investorMoney(investorField(a,'accrued_interest','accrued_unpaid_interest'))}</td><td><span class="badge">${escapeHtml(a.status||'ACTIVE')}</span></td><td><button data-agreement-detail="${escapeHtml(a.id)}">View</button> <button data-agreement-edit="${escapeHtml(a.id)}">Edit</button> <button data-record-funding="${escapeHtml(a.id)}">Record Funding</button> <button data-repay-principal="${escapeHtml(a.id)}">Repay Principal</button> <button data-accrue-interest="${escapeHtml(a.id)}">Accrue Interest</button> <button data-pay-interest="${escapeHtml(a.id)}">Pay Interest</button> <button data-close-agreement="${escapeHtml(a.id)}">Close Agreement</button></td></tr>`).join('')||'<tr><td colspan="12">No funding agreements found.</td></tr>'}</tbody></table></div>`; }catch(e){root.innerHTML=`<div class="alert error">${escapeHtml(investorApiError(e))}</div>`;} }
async function loadRecordFundingPage(){ const root=document.querySelector('#investor-funding-record-root'); if(!root)return; root.innerHTML='<h2>Record Investor Funding</h2><p>Loading...</p>'; try{ await investorBootstrap(); const bankOpts=investorOptions(investorFundingState.accounts.filter(investorIsBank), investorAccountLabel); root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Investor Funding</div><h2>Record Investor Funding</h2></div></div>${investorConfigWarning()}<div id="funding-msg"></div><div class="accounting-grid"><label>Investor<select id="fund-investor"><option value="">Select investor</option>${investorOptions(investorFundingState.investors,r=>investorField(r,'investor_name','name','full_name','company_name'))}</select></label><label>Agreement<select id="fund-agreement"><option value="">Select agreement</option>${investorOptions(investorFundingState.agreements,a=>`${investorField(a,'agreement_number','number','id')} — ${investorField(a,'agreement_name','name','investor_name')}`)}</select></label><label>Transaction date<input id="fund-date" type="date" value="${investorToday()}"></label><label>Amount<input id="fund-amount" type="number" step="0.01" min="0" value="1000000"></label><label>Receiving bank account<select id="fund-bank">${bankOpts}</select></label><label>Reference<input id="fund-reference"></label><label>Remarks<textarea id="fund-remarks"></textarea></label></div><div id="historical-funding-panel"></div><div id="funding-preview" class="subcard"></div><div id="funding-result"></div><div class="modal-actions sticky-modal-footer"><button id="preview-funding" class="secondary">Preview</button><button id="confirm-funding">Confirm Funding</button><button class="secondary" data-section-link="investor-funding-agreements">Cancel</button></div>`; wireFundingForm(root); }catch(e){root.innerHTML=`<div class="alert error">${escapeHtml(investorApiError(e))}</div>`;} }
function wireFundingForm(root){ const els=['#fund-date','#fund-amount','#fund-bank','#fund-reference','#fund-remarks','#fund-agreement'].map(s=>root.querySelector(s)); const preview=root.querySelector('#funding-preview'), hist=root.querySelector('#historical-funding-panel'); const render=()=>{ const amount=Number(root.querySelector('#fund-amount').value||0); const bank=root.querySelector('#fund-bank option:checked')?.textContent||'1010 Main Bank Account'; const date=root.querySelector('#fund-date').value; if(isHistoricalDate(date)){ const today=investorToday(); const months=Math.max(0,(new Date(today).getFullYear()-new Date(date).getFullYear())*12+new Date(today).getMonth()-new Date(date).getMonth()); hist.innerHTML=`<div class="alert warning"><strong>Historical investor funding</strong><br>Transaction date: ${escapeHtml(date)}<br>Current date: ${escapeHtml(today)}<br>Completed months: ${months}<br>Estimated historical accrual periods: ${months}<br>Accounting-period status: verify open period.<br>Historical month-end interest journals may be created for completed periods.</div>`; } else hist.innerHTML=''; preview.innerHTML=`<h3>Accounting preview</h3><div class="accounting-grid"><p><strong>Debit</strong><br>${escapeHtml(bank)}<br>${investorMoney(amount)}</p><p><strong>Credit</strong><br>2300 Investor Borrowings<br>${investorMoney(amount)}</p></div><p>Dr Main Bank Account<br>Cr Investor Borrowings</p>`; }; els.forEach(e=>e&&e.addEventListener('input',render)); render(); root.querySelector('#preview-funding').onclick=render; root.querySelector('#confirm-funding').onclick=async()=>{ const payload={investor_id:root.querySelector('#fund-investor').value,agreement_id:root.querySelector('#fund-agreement').value,transaction_date:root.querySelector('#fund-date').value,amount:Number(root.querySelector('#fund-amount').value||0),receiving_bank_account_id:root.querySelector('#fund-bank').value,reference:root.querySelector('#fund-reference').value,remarks:root.querySelector('#fund-remarks').value}; try{ const res=await api(`${investorFundingRoutes.transactions}/funding`,{method:'POST',body:payload}); root.querySelector('#funding-result').innerHTML=`<div class="alert success"><strong>Investor funding recorded successfully.</strong><br>Transaction number: ${escapeHtml(res.transaction_number||res.id||'—')}<br>Investor: ${escapeHtml(res.investor_name||root.querySelector('#fund-investor option:checked')?.textContent||'—')}<br>Agreement: ${escapeHtml(res.agreement_number||root.querySelector('#fund-agreement option:checked')?.textContent||'—')}<br>Amount: ${investorMoney(res.amount||payload.amount)}<br>Bank account: ${escapeHtml(res.bank_account_name||root.querySelector('#fund-bank option:checked')?.textContent||'—')}<br>Journal number: ${escapeHtml(res.journal_number||'—')}<br>New principal balance: ${investorMoney(res.new_principal_balance)}</div>`; }catch(e){root.querySelector('#funding-msg').innerHTML=`<div class="alert error">${escapeHtml(investorApiError(e))}</div>`;} }; }
...
async function loadBalancesPage(){ const root=document.querySelector('#investor-funding-balances-root'); if(!root)return; await investorReport(root,'Investor Balance Report', investorFundingRoutes.balances, `<label>As-of date<input name="as_of_date" type="date" value="${investorToday()}"></label><label>Investor<input name="investor"></label><label>Agreement<input name="agreement"></label><label>Status<select name="status"><option></option><option>ACTIVE</option><option>CLOSED</option></select></label>`, ['Investor','Agreement','Opening Principal','Funding Added','Principal Repaid','Capitalized Interest','Closing Principal','Accrued Interest','Total Liability']); }
...
function injectInvestorFundingSettingsPanel(){
  const root=document.querySelector('#accounting-settings-root');
  if(!root || root.querySelector('[data-investor-settings-panel]')) return;
  root.insertAdjacentHTML('beforeend', `<div class="subcard" data-investor-settings-panel><h3>Investor Funding</h3><p class="muted">Default mappings for investor borrowings, interest accruals, payments, capitalization, and reconciliation.</p><div class="accounting-grid">${['Investor Borrowings Control Account','Investor Interest Expense Account','Accrued Investor Interest Payable Account','Withholding Tax Payable Account','Default Funding Bank Account','Default Interest Calculation Method','Default Interest Rate Period','Auto-post Investor Interest','Allow Historical Investor Transactions','Allow Interest Capitalization','Reconciliation Tolerance'].map(label=>`<label>${label}<input data-investor-setting="${escapeHtml(label.toLowerCase().replaceAll(' ','_').replaceAll('-','_'))}" placeholder="${escapeHtml(label)}"></label>`).join('')}</div><div class="alert warning"><strong>Investor funding accounting configuration is incomplete.</strong><br>Configure required investor accounts before posting investor funding, accruals, payments, or capitalization.</div><button data-save-investor-settings>Save Investor Funding Settings</button></div>`);
}
document.addEventListener('click', e=>{
  const status=e.target.closest('[data-investor-status]');
  if(status){ e.preventDefault(); api(`${investorFundingRoutes.investors}/${encodeURIComponent(status.dataset.investorStatus)}/status`,{method:'PATCH',body:{status:status.dataset.status}}).then(loadInvestorsPage).catch(err=>alert(investorApiError(err))); }
  ...
  if(saveSettings){ e.preventDefault(); const panel=document.querySelector('[data-investor-settings-panel]'); const body=Object.fromEntries([...panel.querySelectorAll('[data-investor-setting]')].map(i=>[i.dataset.investorSetting,i.value])); api(investorFundingRoutes.settings,{method:'PATCH',body}).then(()=>alert('Investor funding settings saved.')).catch(err=>alert(investorApiError(err))); }
});
