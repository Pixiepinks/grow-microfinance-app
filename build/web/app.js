...
const bankReconciliationState={current:null,currentId:null,transactions:[],transactionsLoading:false,transactionsError:null,requestSequence:0,selected:new Set(),accounts:[]};
...
function normalizeBankGlLine(row={}){return {id:row.journal_line_id??row.line_id??row.id,journalEntryId:row.journal_entry_id??row.entry_id??null,journalNumber:row.journal_number??row.journal_no??row.entry_number??'—',date:row.posting_date??row.journal_date??row.transaction_date??row.date??null,description:row.description??row.narration??row.memo??'—',reference:row.reference??row.reference_number??row.source_reference??'—',debit:row.debit??row.debit_amount??'0.00',credit:row.credit??row.credit_amount??'0.00',runningBalance:row.running_balance??row.balance??'0.00',isReconciled:row.is_reconciled===true||row.reconciled===true||String(row.reconciliation_status||'').toUpperCase()==='RECONCILED',raw:row};}
