...
function agreementForm(existing={}){
  ...
  function validateAgreement(payload){ if(!payload.investor_id||!Number.isInteger(payload.investor_id)) return 'Select an investor.'; if(!payload.agreement_name.trim()) return 'Agreement name is required.'; if(!payload.agreement_date) return 'Agreement date is required.'; if(!payload.start_date) return 'Start date is required.'; if(!(payload.original_principal_amount>0)) return 'Principal expectation must be greater than zero.'; if(!Number.isFinite(payload.interest_rate)||payload.interest_rate<0) return 'Interest rate must be valid.'; if(!payload.interest_rate_period) return 'Interest rate period is required.'; if(!payload.calculation_method) return 'Interest calculation method is required.'; if(!payload.funding_account_id) return 'Funding account is required.'; if(!payload.investor_liability_account_id) return 'Investor liability account is required.'; if(!payload.interest_expense_account_id) return 'Interest expense account is required.'; if(!payload.accrued_interest_payable_account_id) return 'Accrued interest payable account is required.'; if(!payload.status) return 'Status is required.'; if(payload.withholding_tax_rate>0&&!payload.withholding_tax_account_id) return 'Withholding tax payable account is required when withholding tax rate is greater than zero.'; return ''; }
