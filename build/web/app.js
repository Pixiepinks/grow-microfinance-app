...
function csRenderSheetModal(modal,sheet={status:'DRAFT'}){
  ...
  const save=async()=>{ const button=modal.querySelector('[data-cs-save]'); button.disabled=true;message.innerHTML='Saving draft…';try{const result=await api(id?`/admin/collection-sheets/${encodeURIComponent(id)}`:'/admin/collection-sheets',{method:id?'PATCH':'POST',body:payload()});const newId=csId(result)||id;if(!newId)throw new Error('The backend did not return a collection sheet ID.');message.innerHTML='<div class="alert success">Draft saved. No customer payments have been posted.</div>';await csRefreshModal(modal,newId);await loadCollectionSheets();return newId;}catch(error){message.innerHTML=`<div class="alert error">${escapeHtml(csError(error,'Failed to save draft.'))}</div>`;}finally{button.disabled=false;}};
  ...
  search?.addEventListener('input',()=>{clearTimeout(collectionSheetState.searchTimer);const q=search.value.trim(),results=modal.querySelector('#cs-loan-results');if(q.length<2){results.innerHTML='';return;}collectionSheetState.searchTimer=setTimeout(async()=>{results.innerHTML='<p>Searching…</p>';try{const loans=collectionItems(await api(`/admin/collection-sheets/loan-search?q=${encodeURIComponent(q)}`));results.innerHTML=`<div class="table-scroll"><table><thead><tr><th>Customer</th><th>NIC</th><th>Loan Number</th><th>Loan Status</th><th>Outstanding</th><th></th></tr></thead><tbody>${loans.map(loan=>`<tr><td>${escapeHtml(csValue(loan,['customer_name','customer'],'—'))}</td><td>${escapeHtml(csValue(loan,['nic','customer_nic'],'—'))}</td><td>${escapeHtml(csValue(loan,['loan_number','number'],'—'))}</td><td>${escapeHtml(csValue(loan,['loan_status','status'],'—'))}</td><td>${formatCurrency(csValue(loan,['outstanding','outstanding_balance'],0))}</td><td><button data-cs-select-loan="${escapeHtml(csValue(loan,['loan_id','id']))}">Select</button></td></tr>`).join('')||'<tr><td colspan="6">No matching loans.</td></tr>'}</tbody></table></div>`;results.querySelectorAll('[data-cs-select-loan]').forEach(button=>button.onclick=()=>{modal.querySelector('#cs-selected-loan-id').value=button.dataset.csSelectLoan;modal.querySelector('#cs-selected-loan').classList.remove('hidden');modal.querySelector('#cs-item-amount').focus();});}catch(error){results.innerHTML=`<div class="alert error">${escapeHtml(csError(error,'Loan search failed.'))}</div>`;}},300);});
  modal.querySelector('[data-cs-add-item]')?.addEventListener('click',async()=>{if(!id){message.innerHTML='<div class="alert warning">Save the draft before adding collections.</div>';return;}const body={loan_id:modal.querySelector('#cs-selected-loan-id').value,amount_collected:Number(modal.querySelector('#cs-item-amount').value)};try{await api(`/admin/collection-sheets/${encodeURIComponent(id)}/items`,{method:'POST',body});await csRefreshModal(modal,id);}catch(error){message.innerHTML=`<div class="alert error">${escapeHtml(csError(error))}</div>`;}});
}
...
showAdminSection=function(section='dashboard'){
  if(section!=='collection-sheets'){collectionSheetsShowAdminSection(section);return;}
  document.querySelectorAll('.admin-section').forEach(el=>el.classList.toggle('hidden',el.dataset.section!==section));
  document.querySelectorAll('.admin-menu-item').forEach(el=>el.classList.toggle('active',el.dataset.section===section||el.dataset.sectionLink===section));
