...
function normalizeInvestor(item){ return { id:Number(item.id??item.investor_id), investorNumber:item.investor_number??'', displayName:item.display_name??item.full_name??item.company_name??item.investor_number??'Investor', investorType:item.investor_type??'—', identification:item.nic??item.company_registration_number??'—', mobile:item.mobile??'—', activeAgreements:Number(item.active_agreements??item.active_agreement_count??0), principalBalance:Number(item.principal_balance??item.current_principal_balance??0), accruedInterest:Number(item.accrued_interest??item.accrued_interest_balance??0), status:String(item.status??'').trim().toUpperCase() }; }
function investorListErrorMessage(e){ if(e?.status===401) return 'Your session has expired. Please sign in again.'; if(e?.status===403) return 'You do not have permission to view investors.'; if(e?.status===404) return 'Investor list endpoint was not found.'; if(e?.status===500) return 'Investors could not be loaded due to a server error.'; return 'Investors could not be loaded.'; }
function investorPageShell(root){ clearInvestorPageChrome(); const contentRoot=root.closest('.admin-content')||document.querySelector('.admin-content'); if(contentRoot){ contentRoot.style.flex='1'; contentRoot.style.minWidth='0'; contentRoot.style.width='auto'; } root.replaceChildren(); root.innerHTML='<section class="page-card investor-page"><div class="card-header"><div><div class="eyebrow">INVESTOR FUNDING</div><h2>Investors</h2><p class="muted">Manage investors, funding agreements, balances, and accrued interest.</p></div><button data-add-investor>Add Investor</button></div><div id="investor-message"></div><div id="investor-page-body"><p>Loading investors...</p></div></section>'; console.log('Investor page root', contentRoot||root); if(history?.pushState && location.pathname!=='/admin/investors') history.pushState({page:'investors'},'', '/admin/investors'); return root.querySelector('#investor-page-body'); }
function renderInvestorListBody(root, investors, filters={search:'',type:'',status:''}){ const page=root.closest('.investor-page'); const body=page.querySelector('#investor-page-body'); const q=String(filters.search||'').toLowerCase(); const rows=investors.filter(i=>(!filters.type||i.investorType===filters.type)&&(!filters.status||i.status===filters.status)&&(!q||[i.investorNumber,i.displayName,i.identification,i.mobile].some(v=>String(v||'').toLowerCase().includes(q)))); const total=investors.length, active=investors.filter(i=>i.status==='ACTIVE').length, agreements=investors.reduce((a,i)=>a+i.activeAgreements,0), principal=investors.reduce((a,i)=>a+i.principalBalance,0), interest=investors.reduce((a,i)=>a+i.accruedInterest,0); const types=[...new Set(investors.map(i=>i.investorType).filter(Boolean).filter(t=>t!=='—'))]; const controls=`<div class="accounting-grid investor-summary-grid">${[['Total Investors',total],['Active Investors',active],['Active Agreements',agreements],['Total Principal',investorMoney(principal)],['Accrued Interest',investorMoney(interest)]].map(([l,v])=>`<div class="metric"><div class="metric-label">${escapeHtml(l)}</div><div class="metric-value">${escapeHtml(String(v??0))}</div></div>`).join('')}</div>${investorConfigWarning()}<div class="accounting-filters investor-filters"><input id="investor-search" placeholder="Search by investor number, name, NIC, mobile" value="${escapeHtml(filters.search||'')}"><select id="investor-type-filter"><option value="">All types</option>${types.map(t=>`<option value="${escapeHtml(t)}" ${filters.type===t?'selected':''}>${escapeHtml(t)}</option>`).join('')}</select><select id="investor-status-filter"><option value="">All statuses</option>${['ACTIVE','INACTIVE','SUSPENDED'].map(st=>`<option value="${st}" ${filters.status===st?'selected':''}>${st}</option>`).join('')}</select><button class="secondary" data-refresh-investors>Refresh</button></div>`; if(!investors.length){ body.innerHTML=controls+'<div class="subcard investor-empty"><h3>No investors found</h3><p>Create the first investor to start recording investor funding agreements.</p><button data-add-investor>Add Investor</button></div>'; } else { body.innerHTML=controls+`<div class="table-scroll collection-responsive-table investor-table"><table><thead><tr>${['Investor Number','Investor Name','Type','NIC / Registration No.','Mobile','Active Agreements','Principal Balance','Accrued Interest','Status','Actions'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr><td data-label="Investor Number">${escapeHtml(r.investorNumber)}</td><td data-label="Investor Name">${escapeHtml(r.displayName)}</td><td data-label="Type"><span class="badge">${escapeHtml(r.investorType)}</span></td><td data-label="NIC / Registration No.">${escapeHtml(r.identification)}</td><td data-label="Mobile">${escapeHtml(r.mobile)}</td><td data-label="Active Agreements">${r.activeAgreements}</td><td data-label="Principal Balance">${investorMoney(r.principalBalance)}</td><td data-label="Accrued Interest">${investorMoney(r.accruedInterest)}</td><td data-label="Status"><span class="badge">${escapeHtml(r.status||'—')}</span></td><td data-label="Actions"><button data-investor-view="${escapeHtml(r.id)}">View</button> <button data-investor-edit="${escapeHtml(r.id)}">Edit</button> <button data-new-agreement="${escapeHtml(r.id)}">New Agreement</button> <button data-record-funding="${escapeHtml(r.id)}">Record Funding</button> <button data-investor-status="${escapeHtml(r.id)}" data-status="${r.status==='ACTIVE'?'INACTIVE':'ACTIVE'}">${r.status==='ACTIVE'?'Deactivate':'Activate'}</button></td></tr>`).join('')||'<tr><td colspan="10">No investors match the selected filters.</td></tr>'}</tbody></table></div>`; }
  ['#investor-search','#investor-type-filter','#investor-status-filter'].forEach(sel=>body.querySelector(sel)?.addEventListener('input',()=>renderInvestorListBody(root, investors, {search:body.querySelector('#investor-search')?.value||'',type:body.querySelector('#investor-type-filter')?.value||'',status:body.querySelector('#investor-status-filter')?.value||''}))); }
async function loadInvestorsPage(){ const root=document.querySelector('#investor-funding-investors-root'); if(!root)return; const previous={search:root.querySelector('#investor-search')?.value||'',type:root.querySelector('#investor-type-filter')?.value||'',status:root.querySelector('#investor-status-filter')?.value||''}; const body=investorPageShell(root); body.innerHTML='<p>Loading investors...</p>'; try{ await loadInvestorFundingSettings(); const response=await api('/admin/investors'); console.log('Investors response', response); const investors=normalizeInvestorList(response).map(normalizeInvestor); console.log('Normalized investors', investors); investorFundingState.investors=investors; renderInvestorListBody(root, investors, previous); }catch(e){ body.innerHTML=`<div class="alert error"><strong>${escapeHtml(investorListErrorMessage(e))}</strong><br><button data-refresh-investors>Retry</button></div>`; } }
...
(function(){const st=document.createElement('style');st.textContent='.admin-content{flex:1;min-width:0;width:auto}.investor-page{width:100%;max-width:none;box-sizing:border-box}.investor-page .card-header{align-items:flex-start}.investor-summary-grid{margin:16px 0}.investor-filters{margin:16px 0}.investor-table table{width:100%}.investor-empty{text-align:center;padding:32px}@media(max-width:700px){.investor-table table,.investor-table thead,.investor-table tbody,.investor-table tr,.investor-table td{display:block;width:100%}.investor-table thead{display:none}.investor-table tr{border:1px solid var(--border,#ddd);border-radius:12px;margin:0 0 12px;padding:8px}.investor-table td{display:flex;justify-content:space-between;gap:10px;border:0}.investor-table td:before{content:attr(data-label);font-weight:700}.investor-page .card-header{display:flex;gap:12px}.investor-page .card-header button{align-self:flex-start}}';document.head.appendChild(st);})();
