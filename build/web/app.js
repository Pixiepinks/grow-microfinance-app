...
window.addEventListener('popstate', () => {
  ...
  if (renderAdminRoute(path)) return;
  if (path === '/lead') {
    showPublicLeadPage();
    return;
  }
...
if (window.location.pathname === '/lead') {
  ...
  setTimeout(() => renderAdminRoute(window.location.pathname), 0);
}
...
document.addEventListener('click', (e)=>{ const routeLink=e.target.closest('[data-admin-route]'); if(routeLink && !e.defaultPrevented && e.button===0 && !e.metaKey && !e.ctrlKey && !e.shiftKey && !e.altKey){ e.preventDefault(); navigateToAdminRoute(routeLink.getAttribute('href')); return; } const sec=e.target.closest('[data-accounting-section],[data-section-link]')?.dataset.accountingSection || e.target.closest('[data-accounting-section],[data-section-link]')?.dataset.sectionLink; if(sec){ e.preventDefault(); if(investorFundingRouteBySection[sec]) navigateToAdminRoute(investorFundingRouteBySection[sec].path); else showAdminSection(sec); } const jid=e.target.closest('[data-journal-id]')?.dataset.journalId; if(jid){ e.preventDefault(); showAdminSection('accounting-journal-detail'); accountingLoadDetail(jid); }});
...
const investorFundingPageRoutes = [
  ['Investors','investor-funding-investors','/admin/investors','investors'],
  ['Funding Agreements','investor-funding-agreements','/admin/investor-agreements','agreements'],
  ['Record Investor Funding','investor-funding-record','/admin/investor-funding','record'],
  ['Interest Accruals','investor-funding-accruals','/admin/investor-interest-accruals','accruals'],
  ['Interest Payments','investor-funding-payments','/admin/investor-interest-payments','payments'],
  ['Investor Balances','investor-funding-balances','/admin/investor-balances','balances'],
  ['Investor Reports','investor-funding-reports','/admin/investor-reports','reports'],
  ['Investor Reconciliation','investor-funding-reconciliation','/admin/investor-reconciliation','reconciliation'],
];
const investorFundingRouteByPath = Object.fromEntries(investorFundingPageRoutes.map(([, section, path, key]) => [path, { section, key }]));
const investorFundingRouteBySection = Object.fromEntries(investorFundingPageRoutes.map(([label, section, path, key]) => [section, { label, path, key }]));
const investorFundingPageMeta = {
  investors: { eyebrow:'Investor Funding', title:'Investors', subtitle:'Manage investors, funding agreements, balances, and accrued interest.' },
  agreements: { eyebrow:'Investor Funding', title:'Funding Agreements', subtitle:'Create and manage investor funding agreements.' },
  record: { eyebrow:'Investor Funding', title:'Record Investor Funding', subtitle:'Record initial and additional investor funds received.' },
  accruals: { eyebrow:'Investor Funding', title:'Interest Accruals', subtitle:'Review and post investor interest accruals.' },
  payments: { eyebrow:'Investor Funding', title:'Interest Payments', subtitle:'Record investor interest settlements.' },
  balances: { eyebrow:'Investor Funding', title:'Investor Balances', subtitle:'Review principal and accrued interest balances.' },
  reports: { eyebrow:'Investor Funding', title:'Investor Reports', subtitle:'Run investor funding and interest reports.' },
  reconciliation: { eyebrow:'Investor Funding', title:'Investor Reconciliation', subtitle:'Reconcile investor subledgers with the General Ledger.' },
};
function renderInvestorFundingPageShell({eyebrow, title, subtitle, actions, body}) { return `<section class="investors-ui-page investor-funding-page" data-layout-version="investor-routes-v4"><header class="investor-page-header"><div><div class="investor-eyebrow">${escapeHtml(eyebrow)}</div><h1>${escapeHtml(title)}</h1><p>${escapeHtml(subtitle)}</p></div><div class="investor-page-actions">${actions || ''}</div></header><div class="investor-page-body">${body}</div></section>`; }
function investorWrapRouteHtml(key, body='', actions='') { return renderInvestorFundingPageShell({ ...(investorFundingPageMeta[key] || investorFundingPageMeta.investors), actions, body }); }
function ensureInvestorRouteShell(root, key, actions='') { if(!root || root.querySelector('[data-layout-version="investor-routes-v4"]')) return; const body=root.innerHTML; root.innerHTML=investorWrapRouteHtml(key, body, actions); }
function ensureInvestorFundingNavigation(){
  const host=document.querySelector('.admin-sidebar,.sidebar,.admin-menu,nav,.admin-nav');
  if(host && !document.querySelector('[data-investor-funding-nav]')) host.insertAdjacentHTML('beforeend', `<div data-investor-funding-nav class="menu-group"><div class="eyebrow">Investor Funding</div>${investorFundingPageRoutes.map(([l,s,path])=>`<a class="admin-menu-item" href="${path}" data-section-link="${s}" data-admin-route="${path}">${l}</a>`).join('')}</div>`);
  const sectionsHost=document.querySelector('.admin-content,#admin-panel,.admin-sections,#dashboards');
  if(sectionsHost) [['investor-funding-investors','investor-funding-investors-root'],['investor-funding-agreements','investor-funding-agreements-root'],['investor-funding-record','investor-funding-record-root'],['investor-funding-accruals','investor-funding-accruals-root'],['investor-funding-payments','investor-funding-payments-root'],['investor-funding-balances','investor-funding-balances-root'],['investor-funding-reports','investor-funding-reports-root'],['investor-funding-reconciliation','investor-funding-reconciliation-root'],['investor-funding-agreement-detail','investor-funding-agreement-detail-root']].forEach(([sec,id])=>{ if(!document.querySelector(`#${id}`)) sectionsHost.insertAdjacentHTML('beforeend', `<section class="admin-section hidden investor-route-section" data-section="${sec}"><div id="${id}"></div></section>`); });
  updateInvestorFundingActiveNav();
}
...
function logExistingShellCheck(){ console.log("Existing shell check", { headers: document.querySelectorAll("header, .site-header, .admin-header").length, sidebars: document.querySelectorAll("aside, .admin-sidebar, .navigation").length, contentRoots: document.querySelectorAll(".admin-content, #main-content, .page-content").length }); }
...
function investorPageShell(root){ clearInvestorPageChrome(); logExistingShellCheck(); const contentRoot=root.closest('.admin-content')||document.querySelector('#admin-content')||document.querySelector('.admin-content')||root; root.replaceChildren(); root.innerHTML=investorWrapRouteHtml('investors','<div class="page-card investor-page investors-dashboard-v2" data-ui-version="investors-v2"><div id="investor-message"></div><div id="investor-page-body"><p class="investor-loading">Loading investors...</p></div></div>','<button type="button" id="add-investor-btn" class="investor-primary-action" data-add-investor>Add Investor</button>'); console.log('Investor page root', contentRoot||root); return root.querySelector('#investor-page-body'); }
...
function updateInvestorFundingActiveNav(){ const visible=document.querySelector('.admin-section:not(.hidden)[data-section^="investor-funding-"]')?.dataset.section || ''; document.querySelectorAll('[data-investor-funding-nav] .admin-menu-item').forEach(link=>{ const active=link.dataset.sectionLink===visible || (!visible && link.getAttribute('href')===window.location.pathname); link.classList.toggle('active', active); link.setAttribute('aria-current', active?'page':'false'); }); }
function cleanupInvestorFundingRoute(){ clearInvestorPageChrome(); document.querySelectorAll('.admin-content,#admin-panel').forEach(el=>{ el.classList.remove('split-view','customer-detail-pane','has-detail-pane'); el.style.gridTemplateColumns=''; el.style.marginLeft=''; el.style.left=''; el.style.maxWidth=''; }); }
function navigateToAdminRoute(path){ const route=investorFundingRouteByPath[path]; if(route){ showAdminSection(route.section); updateInvestorFundingActiveNav(); } }
function renderAdminRoute(pathname=window.location.pathname){ const route=investorFundingRouteByPath[pathname]; if(route){ ensureInvestorFundingNavigation(); cleanupInvestorFundingRoute(); showAdminSection(route.section); updateInvestorFundingActiveNav(); return true; } return false; }
...
document.addEventListener('click', async e=>{
  ...
  const rec=e.target.closest('[data-record-funding]'); if(rec){ e.preventDefault(); navigateToAdminRoute('/admin/investor-funding'); setTimeout(()=>{ const el=document.querySelector('#fund-agreement'); if(el) el.value=rec.dataset.recordFunding; },100); return; }
