...
const investorFundingState = { investors: [], agreements: [], accounts: [], settings: normalizeInvestorFundingSettings({}), settingsError: null, settingsLoaded: false, settingsLoading: false, settingsPromise: null, lastFundingResult: null, investorOptionsCache: null, accrualMonth: '', accrualRows: [], accrualMode: 'HISTORICAL', accrualRequestSequence: 0 };
...
function defaultInvestorAccrualMonth(){ const [year,month]=investorToday().slice(0,7).split('-').map(Number); return month===1?`${year-1}-12`:`${year}-${String(month-1).padStart(2,'0')}`; }
function accrualResponseRows(response){ if(Array.isArray(response))return response; const candidates=[response?.accruals,response?.results,response?.preview,response?.preview_rows,response?.rows,response?.items,response?.data]; for(const candidate of candidates){ if(Array.isArray(candidate))return candidate; if(candidate&&candidate!==response){ const nested=accrualResponseRows(candidate); if(nested.length)return nested; } } return []; }
function accrualRowRate(row){ const value=row.interest_rate ?? row.monthly_interest_rate ?? row.rate_percent ?? row.rate; return investorRate(value,row.interest_rate_period ?? row.rate_period ?? 'MONTHLY'); }
function accrualMonthLabel(month){ if(!/^\d{4}-\d{2}$/.test(month||''))return month||'selected month'; const [year,number]=month.split('-').map(Number); return `${['January','February','March','April','May','June','July','August','September','October','November','December'][number-1]} ${year}`; }
function renderAccrualDashboard(root,d){ const box=root.querySelector('#investor-accrual-dashboard'); if(!box)return; box.innerHTML=[['Agreements requiring accrual',d.agreements_requiring_accrual],['Accruals posted this month',d.accruals_posted_this_month],['Accruals awaiting payment',d.accruals_awaiting_payment],['Accrual exceptions',d.accrual_exceptions],['Total investor principal',investorMoney(d.total_investor_principal)],['Total accrued interest',investorMoney(d.total_accrued_interest)]].map(([l,v])=>`<div class="metric"><div class="metric-label">${escapeHtml(l)}</div><div>${escapeHtml(String(v??'0'))}</div></div>`).join(''); }
function renderAccrualRows(root,rows,mode,month){ investorFundingState.accrualRows=rows; investorFundingState.accrualMode=mode; investorFundingState.accrualMonth=month; const status=root.querySelector('#investor-accrual-table-status'),box=root.querySelector('#investor-accrual-table'); if(status)status.innerHTML=mode==='PREVIEW'?`<div class="alert info"><strong>Preview for ${escapeHtml(accrualMonthLabel(month))} — not posted</strong></div>`:''; if(!box)return; box.innerHTML=investorTable(['Accrual Month','Investor','Agreement','Opening Principal','Average Daily Balance','Rate','Gross Interest','Withholding Tax','Net Payable','Paid','Outstanding','Status','Journal','Actions'],rows.map(r=>{ const preview=mode==='PREVIEW'; return [r.accrual_month??r.month??month,r.investor_name??r.investor?.display_name??r.investor?.name,r.agreement_number??r.agreement?.agreement_number,investorMoney(r.opening_principal??r.opening_principal_balance),investorMoney(r.average_daily_balance??r.avg_daily_balance),accrualRowRate(r),investorMoney(r.gross_interest??r.interest_amount),investorMoney(r.withholding_tax??r.withholding_tax_amount),investorMoney(r.net_payable??r.net_interest_payable),investorMoney(preview?0:r.paid),investorMoney(r.outstanding??r.net_payable??r.net_interest_payable),preview?'PREVIEW':r.status,preview?'—':(r.journal_number??r.journal_no??'—'),preview?'—':`<button>View</button> <button data-pay-accrual="${escapeHtml(r.id)}">Pay</button> <button data-capitalize-interest="${escapeHtml(r.id)}">Capitalize</button> <button data-investor-reverse="${escapeHtml(r.id)}" data-kind="accrual">Reverse</button>`]; })); }
async function refreshAccrualDashboard(root,month){ const d=await api(`${investorFundingRoutes.accruals}/dashboard?month=${encodeURIComponent(month)}`); if(root.isConnected&&root.querySelector('#run-month')?.value===month)renderAccrualDashboard(root,d?.data??d); }
async function loadAccrualHistory(root,month,requestId=++investorFundingState.accrualRequestSequence){ const response=await api(`${investorFundingRoutes.accruals}?month=${encodeURIComponent(month)}`); if(requestId!==investorFundingState.accrualRequestSequence||root.querySelector('#run-month')?.value!==month)return; renderAccrualRows(root,accrualResponseRows(response),'HISTORICAL',month); }
async function runInvestorAccrual(root,button){ const month=root.querySelector('#run-month').value,agreementValue=root.querySelector('#run-agreement').value.trim(),previewOnly=root.querySelector('#run-preview').checked,post=root.querySelector('#run-post').checked,message=root.querySelector('#investor-accrual-message'); if(!/^\d{4}-\d{2}$/.test(month)){setInlineAlert(message,'Select a valid accrual month.','error');return;} if(previewOnly===post){setInlineAlert(message,'Choose either Preview only or Post.','error');return;} const requestId=++investorFundingState.accrualRequestSequence; button.disabled=true; button.textContent='Running...'; setInlineAlert(message,''); const body={month,agreement_id:agreementValue||null,preview_only:previewOnly,post}; try{ const response=await api(`${investorFundingRoutes.accruals}/run`,{method:'POST',body}); if(requestId!==investorFundingState.accrualRequestSequence)return; const rows=accrualResponseRows(response); if(previewOnly){ renderAccrualRows(root,rows,'PREVIEW',month); setInlineAlert(message,rows.length?`Interest accrual preview generated for ${accrualMonthLabel(month)}. ${rows.length} agreement(s) calculated. Nothing has been posted.`:`No eligible investor agreements require accrual for ${accrualMonthLabel(month)}.`,'success'); }else{ if(rows.length)renderAccrualRows(root,rows,'POSTED',month); else await loadAccrualHistory(root,month,requestId); setInlineAlert(message,`${accrualMonthLabel(month)} interest accrual posted successfully.`,'success'); } await refreshAccrualDashboard(root,month); }catch(error){ console.error('Unable to run investor interest accrual',error); setInlineAlert(message,`Unable to run interest accrual: ${investorApiError(error)}`,'error'); }finally{ if(root.isConnected){button.disabled=false;button.textContent='Run Interest Accrual';} } }
async function loadAccrualsPage(){ const root=document.querySelector('#investor-funding-accruals-root'); if(!root)return; const month=investorFundingState.accrualMonth||defaultInvestorAccrualMonth(); root.innerHTML='<h2>Interest Accruals</h2><p>Loading...</p>'; try{ const [dash,rowsRes,agreementsRes]=await Promise.allSettled([api(`${investorFundingRoutes.accruals}/dashboard?month=${encodeURIComponent(month)}`),api(`${investorFundingRoutes.accruals}?month=${encodeURIComponent(month)}`),api(investorFundingRoutes.agreements)]); const agreements=investorItems(agreementsRes.value).map(normalizeFundingAgreement); root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Investor Funding</div><h2>Interest Accruals</h2></div></div><div id="investor-accrual-dashboard" class="accounting-grid"></div><div class="subcard"><h3>Run Interest Accrual</h3><div id="investor-accrual-message"></div><div class="accounting-grid"><label>Month<input id="run-month" type="month" value="${escapeHtml(month)}"></label><label>Agreement optional<select id="run-agreement"><option value="">All eligible agreements</option>${agreements.map(a=>`<option value="${escapeHtml(a.id)}">${escapeHtml(`${a.agreementNumber} — ${a.investorName}`)}</option>`).join('')}</select></label><label><input id="run-preview" type="checkbox" checked> Preview only</label><label><input id="run-post" type="checkbox"> Post</label></div><button data-run-investor-accrual>Run Interest Accrual</button></div><div id="investor-accrual-table-status"></div><div id="investor-accrual-table"></div>`; renderAccrualDashboard(root,dash.value?.data??dash.value??{}); renderAccrualRows(root,accrualResponseRows(rowsRes.value),'HISTORICAL',month); const monthEl=root.querySelector('#run-month'),preview=root.querySelector('#run-preview'),post=root.querySelector('#run-post'); preview.onchange=()=>{if(preview.checked)post.checked=false;}; post.onchange=()=>{if(post.checked)preview.checked=false;}; monthEl.onchange=async()=>{const selected=monthEl.value;if(!/^\d{4}-\d{2}$/.test(selected))return; investorFundingState.accrualMonth=selected; const requestId=++investorFundingState.accrualRequestSequence; renderAccrualRows(root,[],'HISTORICAL',selected); setInlineAlert(root.querySelector('#investor-accrual-message'),'Loading selected month...','success'); await Promise.allSettled([refreshAccrualDashboard(root,selected),loadAccrualHistory(root,selected,requestId)]); if(requestId===investorFundingState.accrualRequestSequence)setInlineAlert(root.querySelector('#investor-accrual-message'),'');}; }catch(e){root.innerHTML=`<div class="alert error">${escapeHtml(investorApiError(e))}</div>`;} }
...
document.addEventListener('click', e=>{
  ...
  if(run){ e.preventDefault(); const root=document.querySelector('#investor-funding-accruals-root'); if(root&&!run.disabled)runInvestorAccrual(root,run); }
