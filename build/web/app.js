...
function normalizeFundingAgreement(item = {}) { return { id: Number(item.id ?? item.agreement_id), agreementNumber: item.agreement_number ?? '—', agreementName: item.agreement_name ?? '—', investorId: Number(item.investor_id ?? item.investor?.id), investorName: item.investor_name ?? item.investor?.display_name ?? item.investor?.full_name ?? item.investor?.company_name ?? '—', investorNumber: item.investor_number ?? item.investor?.investor_number ?? '', startDate: item.start_date ?? item.agreement_date ?? null, maturityDate: item.maturity_date ?? null, originalPrincipal: safeNumber(item.original_principal_amount ?? item.original_expected_principal ?? 0), currentPrincipal: safeNumber(item.current_principal_balance ?? item.principal_balance ?? 0), interestRate: safeNumber(item.interest_rate ?? 0), ratePeriod: String(item.interest_rate_period ?? '').trim().toUpperCase(), calculationMethod: String(item.calculation_method ?? '').trim().toUpperCase(), accruedInterest: safeNumber(item.accrued_interest ?? item.accrued_interest_balance ?? 0), status: String(item.status ?? 'DRAFT').trim().toUpperCase(), raw: item }; }
