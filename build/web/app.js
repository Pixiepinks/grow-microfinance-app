...
function investorBalanceErrorMessage(e){ if(e?.status===404) return 'The selected investor or agreement no longer exists. Refresh the options and try again.'; if(e?.status===500) return 'Investor balance report could not be generated.'; if(e?.status===401) return 'Your session has expired. Please sign in again.'; if(e?.status===403) return 'You do not have permission to view the Investor Balance Report.'; return e?.message||'Investor balance report could not be generated.'; }
function investorBalanceRows(response){ return investorItems(response); }
function investorBalanceSummary(response,rows){ const summary=response?.summary??response?.totals??{}; const uniqueInvestors=new Set(rows.map(r=>r.investor_id??r.investorId??r.investor_number).filter(v=>v!=null&&v!=='')); const uniqueAgreements=new Set(rows.map(r=>r.agreement_id??r.agreementId??r.agreement_number).filter(v=>v!=null&&v!=='')); return {totalInvestors:safeNumber(summary.total_investors??summary.totalInvestors??uniqueInvestors.size),totalAgreements:safeNumber(summary.total_agreements??summary.totalAgreements??uniqueAgreements.size),principalBalance:safeNumber(summary.principal_balance??summary.current_principal??summary.current_principal_balance??rows.reduce((t,r)=>t+safeNumber(r.current_principal??r.current_principal_balance??r.principal_balance),0)),accruedInterest:safeNumber(summary.accrued_interest??summary.accrued_interest_balance??rows.reduce((t,r)=>t+safeNumber(r.accrued_interest??r.accrued_interest_balance),0)),totalPayable:safeNumber(summary.total_payable??summary.total_liability??rows.reduce((t,r)=>t+safeNumber(r.total_payable??r.total_liability),0))}; }
async function loadBalancesPage(){
  const root=document.querySelector('#investor-funding-balances-root'); if(!root)return;
  root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Investor Funding</div><h2>Investor Balance Report</h2></div></div><form class="accounting-filters"><label>As-of date<input name="as_of_date" type="date" value="${investorToday()}"></label><label>Investor<select name="investor_id" id="balance-investor"><option value="">All investors</option></select></label><label>Agreement<select name="agreement_id" id="balance-agreement"><option value="">All agreements</option></select></label><label>Status<select name="status"><option value="">All statuses</option><option>ACTIVE</option><option>CLOSED</option></select></label><button>Run Report</button></form><div class="report-results"><div class="alert info">Loading report options...</div></div>`;
  const form=root.querySelector('form'), box=root.querySelector('.report-results'), investorSelect=root.querySelector('#balance-investor'), agreementSelect=root.querySelector('#balance-agreement');
  let investors=[], agreements=[];
  ...
  const renderReport=(response)=>{ const rows=investorBalanceRows(response); if(!rows.length){ box.innerHTML='<div class="alert info">No investor balances found for the selected filters.</div>'; return; } const summary=investorBalanceSummary(response,rows); const cards=[['Total Investors',summary.totalInvestors],['Total Agreements',summary.totalAgreements],['Principal Balance',investorMoney(summary.principalBalance)],['Accrued Interest',investorMoney(summary.accruedInterest)],['Total Payable',investorMoney(summary.totalPayable)]]; const headers=['Investor Number','Investor Name','Agreement Number','Agreement Name','Status','Original Principal','Total Funding','Principal Repaid','Current Principal','Accrued Interest','Interest Paid','Total Payable']; box.innerHTML=`<div class="accounting-grid">${cards.map(([l,v])=>`<div class="metric"><div class="metric-label">${escapeHtml(l)}</div><div>${escapeHtml(String(v??'0'))}</div></div>`).join('')}</div>${investorTable(headers, rows.map(r=>[escapeHtml(r.investor_number??r.investorNumber??'—'),escapeHtml(r.investor_name??r.investorName??'—'),escapeHtml(r.agreement_number??r.agreementNumber??'—'),escapeHtml(r.agreement_name??r.agreementName??'—'),escapeHtml(r.status??'—'),investorMoney(r.original_principal??r.original_principal_amount),investorMoney(r.total_funding??r.funding_added??r.posted_principal),investorMoney(r.principal_repaid),investorMoney(r.current_principal??r.current_principal_balance??r.principal_balance),investorMoney(r.accrued_interest??r.accrued_interest_balance),investorMoney(r.interest_paid),investorMoney(r.total_payable??r.total_liability)]))}`; };
  const run=async(e)=>{ e&&e.preventDefault(); const as_of_date=form.querySelector('[name="as_of_date"]').value; const investor_id=investorSelect.value?Number(investorSelect.value):null; const agreement_id=agreementSelect.value?Number(agreementSelect.value):null; const status=form.querySelector('[name="status"]').value||null; console.log('Investor balance report request',{as_of_date,investor_id,agreement_id,status}); try{ const params=new URLSearchParams(); params.set('as_of_date',as_of_date); if(investor_id!=null) params.set('investor_id',String(investor_id)); if(agreement_id!=null) params.set('agreement_id',String(agreement_id)); if(status) params.set('status',status); const response=await api(`${investorFundingRoutes.balances}?${params.toString()}`); console.log('Investor balance report response',response); renderReport(response); }catch(e){ box.innerHTML=`<div class="alert error">${escapeHtml(investorBalanceErrorMessage(e))}</div>`; } };
  form.onsubmit=run;
  try{ const [investorResponse,agreementResponse]=await Promise.all([api(investorFundingRoutes.investors),api('/admin/investor-agreements')]); investors=normalizeInvestorBalanceInvestors(investorResponse); agreements=normalizeInvestorBalanceAgreements(agreementResponse); investorSelect.innerHTML='<option value="">All investors</option>'+investors.map(i=>`<option value="${escapeHtml(i.id)}">${escapeHtml(investorBalanceOptionText(i.investorNumber,i.displayName,'Investor'))}</option>`).join(''); renderAgreementOptions(); await run(); }catch(e){ box.innerHTML=`<div class="alert error">${escapeHtml(investorBalanceErrorMessage(e))}</div>`; }
}
