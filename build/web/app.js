...
async function recordAdminLedgerPayment(entryId) {
  ...
  const [settingsRaw, accountsRaw, collectorsRaw] = await Promise.allSettled([
    ...
    api('/admin/collectors?active=true'),
  ]);
  ...
  const isCollection=a=>String(acctSubtype(a)||'').toUpperCase().replace(/[ -]/g,'_').includes('COLLECTION_CLEARING') || String(a.name||a.account_name||'').toUpperCase().includes('COLLECTION ACCOUNT');
...
async function collectionBootstrapData(){
  const [settings, accounts, collectors] = await Promise.allSettled([api('/admin/accounting/settings'), api('/admin/accounting/accounts?active=true'), api('/admin/collectors?active=true')]);
...
const collectionsShowAdminSection=showAdminSection; showAdminSection=function(section='dashboard'){ ensureCollectionsNavigation(); collectionsShowAdminSection(section); if(section==='collections-undeposited')loadUndepositedCollections(); if(section==='collections-deposit')loadDepositCollections(); if(section==='collections-register')loadDepositRegister(); if(section==='collections-balances')loadCollectorBalances(); };
