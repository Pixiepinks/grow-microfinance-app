...
const bankReconciliationState={current:null,selected:new Set(),accounts:[]};
...
function bankLineItems(d){for(const k of ['transactions','gl_transactions','bank_transactions','lines','items'])if(Array.isArray(d?.[k]))return d[k];return bankReconciliationItems(d);}
...
function isLineReconciled(line){return line.reconciled===true||line.is_reconciled===true||String(line.reconciliation_status||line.reconciled_status||'').toUpperCase()==='RECONCILED';}
function reconciliationDetail(line){return {number:brField(line,['reconciliation_no','reconciliation_number','bank_reconciliation_no'],'—'),date:brField(line,['reconciled_date','reconciled_at'],'—'),reference:brField(line,['statement_reference','bank_statement_reference','reference'],'—'),by:brField(line,['reconciled_by_name','reconciled_by'],'—')};}
...
async function openBankReconciliation(id,{print=false}={}){const root=document.querySelector('#accounting-bank-reconciliation-root');showAdminSectionWithBankReconciliation('accounting-bank-reconciliation');if(location.pathname!=='/admin/accounting/bank-reconciliation')history.pushState({section:'accounting-bank-reconciliation'},'', '/admin/accounting/bank-reconciliation');root.innerHTML='<h2>Bank Reconciliation</h2><p>Loading workspace...</p>';try{const raw=await api(`${bankReconciliationApi}/${encodeURIComponent(id)}`),r=raw.reconciliation||raw.data||raw;r.transactions=r.transactions||raw.transactions||raw.gl_transactions||r.gl_transactions||[];bankReconciliationState.current=r;bankReconciliationState.selected.clear();renderBankReconciliationWorkspace(root,r);if(print)setTimeout(()=>window.print(),100);}catch(e){root.innerHTML=`<div class="alert error">${escapeHtml(bankReconciliationError(e))}</div>`;}}
function renderBankReconciliationWorkspace(root,r){const rows=bankLineItems(r),status=String(r.status||'DRAFT').toUpperCase(),editable=status!=='COMPLETED',difference=Number(brField(r,['difference'],0))||0;root.innerHTML=`<div class="card-header"><div><div class="eyebrow">Bank Reconciliation</div><h2>${escapeHtml(reconciliationNumber(r))}</h2><p>${escapeHtml(brField(r,['bank_account_label','bank_account_name','account_name'],'—'))} · ${escapeHtml(formatDateOnlyDisplay(brField(r,['statement_date_from','date_from'])))} – ${escapeHtml(formatDateOnlyDisplay(brField(r,['statement_date_to','date_to'])))} · ${brStatus(status)}</p></div><div data-no-print><button class="secondary" data-br-list>Back</button> <button class="secondary" data-br-print-current>Print Reconciliation</button> <button class="secondary" data-br-csv>Export CSV</button></div></div><div id="bank-reconciliation-message"></div><div class="reconciliation-print-meta"><p>Prepared By: ${escapeHtml(brField(r,['prepared_by_name','prepared_by'],'—'))} · Completed By: ${escapeHtml(brField(r,['completed_by_name','completed_by'],'—'))}</p></div><div class="summary-grid">${reconciliationTotals(r).map(([l,v])=>`<div class="summary-card"><span>${l}</span><strong class="${l==='Difference'&&Number(v)!==0?'difference-error':''}">${formatCurrency(v)}</strong></div>`).join('')}</div><div class="table-scroll"><table><thead><tr>${['Select','Date','Journal No.','Description','Reference','Debit','Credit','Running Balance','Reconciled'].map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.length?rows.map(line=>{const reconciled=isLineReconciled(line),lineId=brField(line,['journal_line_id','line_id','id']);const currentNo=reconciliationDetail(line).number;return `<tr><td>${editable&&!reconciled?`<input type="checkbox" data-br-line="${escapeHtml(lineId)}" aria-label="Select bank GL line">`:'—'}</td><td>${escapeHtml(formatDateOnlyDisplay(brField(line,['date','journal_date']))||'—')}</td><td>${escapeHtml(brField(line,['journal_no','journal_number'],'—'))}</td><td>${escapeHtml(line.description||'')}</td><td>${escapeHtml(brField(line,['reference','statement_reference'],'—'))}</td><td>${moneyCell(line.debit)}</td><td>${moneyCell(line.credit)}</td><td>${moneyCell(line.running_balance)}</td><td>${reconciled?`${renderGlReconciliation(line)}${currentNo!=='—'?`<br><small>${escapeHtml(currentNo)}</small>`:''}`:'<span class="reconciliation-status unreconciled">UNRECONCILED</span>'}${editable&&reconciled&&String(currentNo)===String(reconciliationNumber(r))?`<br><button class="link-button" data-br-unmatch="${escapeHtml(lineId)}">Remove Match</button>`:''}</td></tr>`;}).join(''):'<tr><td colspan="9">No bank GL transactions found for this period.</td></tr>'}</tbody></table></div><div class="sticky-actions" data-no-print>${editable?`<button id="mark-bank-reconciled" disabled>Mark Reconciled</button><button id="complete-bank-reconciliation" ${difference!==0?'disabled':''}>Complete Reconciliation</button>`:'<span class="muted">Completed reconciliations are read-only.</span>'}</div>`;root.querySelector('[data-br-list]').onclick=accountingLoadBankReconciliation;root.querySelector('[data-br-print-current]').onclick=()=>window.print();root.querySelector('[data-br-csv]').onclick=()=>exportBankReconciliationCsv(r,rows);root.querySelectorAll('[data-br-line]').forEach(cb=>cb.onchange=()=>{cb.checked?bankReconciliationState.selected.add(cb.dataset.brLine):bankReconciliationState.selected.delete(cb.dataset.brLine);root.querySelector('#mark-bank-reconciled').disabled=!bankReconciliationState.selected.size;});const mark=root.querySelector('#mark-bank-reconciled');if(mark)mark.onclick=()=>updateBankMatches('add',[...bankReconciliationState.selected]);const complete=root.querySelector('#complete-bank-reconciliation');if(complete)complete.onclick=()=>completeBankReconciliation(r);}
async function updateBankMatches(action,lineIds){const r=bankReconciliationState.current,root=document.querySelector('#accounting-bank-reconciliation-root');try{await api(`${bankReconciliationApi}/${encodeURIComponent(reconciliationId(r))}/matches`,{method:action==='add'?'POST':'DELETE',body:{journal_line_ids:lineIds,line_ids:lineIds}});await openBankReconciliation(reconciliationId(r));window.dispatchEvent(new CustomEvent('bank-reconciliation-updated',{detail:{journalLineIds:lineIds}}));}catch(e){root.querySelector('#bank-reconciliation-message').innerHTML=`<div class="alert error">${escapeHtml(bankReconciliationError(e))}</div>`;}}
async function completeBankReconciliation(r){const difference=Number(brField(r,['difference'],0))||0,root=document.querySelector('#accounting-bank-reconciliation-root');if(difference!==0){root.querySelector('#bank-reconciliation-message').innerHTML='<div class="alert error">Bank reconciliation cannot be completed while a difference remains.</div>';return;}const closing=brField(r,['statement_closing_balance'],0),gl=brField(r,['reconciled_gl_closing_balance','gl_closing_balance'],0);if(!confirm(`Complete reconciliation?\nStatement Closing Balance: ${formatCurrency(closing)}\nReconciled GL Closing Balance: ${formatCurrency(gl)}\nDifference: ${formatCurrency(difference)}`))return;try{await api(`${bankReconciliationApi}/${encodeURIComponent(reconciliationId(r))}/complete`,{method:'POST'});await openBankReconciliation(reconciliationId(r));}catch(e){root.querySelector('#bank-reconciliation-message').innerHTML=`<div class="alert error">${escapeHtml(bankReconciliationError(e))}</div>`;}}
function exportBankReconciliationCsv(r,rows){const fields=['Date','Journal No.','Description','Reference','Debit','Credit','Running Balance','Reconciled','Reconciliation No.'],quote=v=>`"${String(v??'').replaceAll('"','""')}"`,csv=[fields,...rows.map(x=>[brField(x,['date','journal_date']),brField(x,['journal_no','journal_number']),x.description,brField(x,['reference','statement_reference']),x.debit,x.credit,x.running_balance,isLineReconciled(x)?'RECONCILED':'UNRECONCILED',reconciliationDetail(x).number])].map(row=>row.map(quote).join(',')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download=`${reconciliationNumber(r)}.csv`;a.click();URL.revokeObjectURL(a.href);}
