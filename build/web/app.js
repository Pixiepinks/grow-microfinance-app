...
function investorPageShell(root){ clearInvestorPageChrome(); const contentRoot=root.closest('.admin-content')||document.querySelector('#admin-content')||document.querySelector('.admin-content')||root; if(contentRoot){ contentRoot.style.flex='1 1 0%'; contentRoot.style.minWidth='0'; contentRoot.style.width='auto'; } root.replaceChildren(); root.innerHTML='<section class="page-card investor-page"><header class="page-header card-header"><div><div class="eyebrow">Investor Funding</div><h1>Investors</h1><p class="muted">Manage investors, funding agreements, balances, and accrued interest.</p></div><button type="button" id="add-investor-btn" data-add-investor>Add Investor</button></header><div id="investor-message"></div><div id="investor-page-body"><p>Loading investors...</p></div></section>'; console.log('Investor page root', contentRoot||root); if(history?.pushState && location.pathname!=='/admin/investors') history.pushState({page:'investors'},'', '/admin/investors'); return root.querySelector('#investor-page-body'); }
function renderInvestorRow(investor) {
  ...
  return `<tr><td>${escapeInvestorHtml(investor.investorNumber)}</td><td><strong>${escapeInvestorHtml(investor.displayName)}</strong></td><td>${escapeInvestorHtml(investor.investorType)}</td><td>${escapeInvestorHtml(investor.identification)}</td><td>${escapeInvestorHtml(investor.mobile)}</td><td class="number-cell">${safeNumber(investor.activeAgreements)}</td><td class="money-cell">${formatLkr(investor.principalBalance)}</td><td class="money-cell">${formatLkr(investor.accruedInterest)}</td><td><span class="status-badge ${statusClass}">${escapeInvestorHtml(investor.status)}</span></td><td><div class="table-actions"><button type="button" class="btn-small view-investor-btn" data-investor-id="${investorId}" ${investorId ? '' : 'disabled'}>View</button><button type="button" class="btn-small edit-investor-btn" data-investor-edit="${investorId}" data-investor-id="${investorId}" ${investorId ? '' : 'disabled'}>Edit</button><button type="button" class="btn-small new-agreement-btn" data-new-agreement="${investorId}" data-investor-id="${investorId}" ${investorId ? '' : 'disabled'}>New Agreement</button></div></td></tr>`;
}
function renderInvestorTable(investors) {
  const rows = Array.isArray(investors) ? investors : [];
  if (rows.length === 0) return `<div class="empty-state investor-empty"><h3>No investors found</h3><p>Create the first investor to begin recording funding agreements.</p><button type="button" id="empty-add-investor" data-add-investor>Add Investor</button></div>`;
  return `<div class="table-scroll collection-responsive-table investor-table"><table class="data-table"><thead><tr>${['Investor Number','Investor Name','Type','NIC / Registration No.','Mobile','Active Agreements','Principal Balance','Accrued Interest','Status','Actions'].map(h=>`<th>${escapeInvestorHtml(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(renderInvestorRow).join('')}</tbody></table></div>`;
}
function renderInvestorSummary(investors){ const totals={ totalInvestors:investors.length, activeInvestors:investors.filter(investor=>investor.status==='ACTIVE').length, activeAgreements:investors.reduce((sum,investor)=>sum+safeNumber(investor.activeAgreements),0), principalBalance:investors.reduce((sum,investor)=>sum+safeNumber(investor.principalBalance),0), accruedInterest:investors.reduce((sum,investor)=>sum+safeNumber(investor.accruedInterest),0) }; return `<div class="summary-grid accounting-grid investor-summary-grid">${[['Total Investors',totals.totalInvestors],['Active Investors',totals.activeInvestors],['Active Agreements',totals.activeAgreements],['Total Principal',formatLkr(totals.principalBalance)],['Accrued Interest',formatLkr(totals.accruedInterest)]].map(([label,value])=>`<div class="metric"><div class="metric-label">${escapeInvestorHtml(label)}</div><div class="metric-value">${escapeInvestorHtml(value)}</div></div>`).join('')}</div>`; }
function renderInvestorListBody(root, investors, filters={search:'',type:'',status:''}){ const page=root.closest('.investor-page')||root.querySelector('.investor-page'); const body=page.querySelector('#investor-page-body'); const q=String(filters.search||'').toLowerCase(); const safeInvestors=Array.isArray(investors)?investors:[]; const rows=safeInvestors.filter(i=>(!filters.type||i.investorType===filters.type)&&(!filters.status||i.status===filters.status)&&(!q||[i.investorNumber,i.displayName,i.identification,i.mobile].some(v=>String(v||'').toLowerCase().includes(q)))); const types=[...new Set(safeInvestors.map(i=>i.investorType).filter(Boolean).filter(t=>t!=='—'))]; const controls=`${renderInvestorSummary(safeInvestors)}${investorConfigWarning()}<div class="filter-bar accounting-filters investor-filters"><input id="investor-search" placeholder="Search by investor number, name, NIC, mobile" value="${escapeInvestorHtml(filters.search||'')}"><select id="investor-type-filter"><option value="">All types</option>${types.map(t=>`<option value="${escapeInvestorHtml(t)}" ${filters.type===t?'selected':''}>${escapeInvestorHtml(t)}</option>`).join('')}</select><select id="investor-status-filter"><option value="">All statuses</option>${['ACTIVE','INACTIVE','SUSPENDED'].map(st=>`<option value="${st}" ${filters.status===st?'selected':''}>${st}</option>`).join('')}</select><button type="button" class="secondary" data-refresh-investors>Refresh</button></div>`; body.innerHTML=controls+`<div id="investor-list-content">${renderInvestorTable(rows)}</div>`; ['#investor-search','#investor-type-filter','#investor-status-filter'].forEach(sel=>body.querySelector(sel)?.addEventListener('input',()=>renderInvestorListBody(root, safeInvestors, {search:body.querySelector('#investor-search')?.value||'',type:body.querySelector('#investor-type-filter')?.value||'',status:body.querySelector('#investor-status-filter')?.value||''}))); }
...
(function(){const st=document.createElement('style');st.textContent='.admin-content{flex:1 1 0%;min-width:0;width:auto}.investor-page{width:100%;max-width:none;min-width:0;box-sizing:border-box}.investor-page-card,.investor-page .page-card{width:100%;max-width:none}.investor-page .card-header,.investor-page .page-header{align-items:flex-start}.investor-summary-grid{margin:16px 0}.investor-filters{margin:16px 0}.investor-table table{width:100%}.investor-table .number-cell,.investor-table .money-cell{text-align:right}.investor-table .table-actions{display:flex;gap:8px;flex-wrap:wrap}.investor-empty{text-align:center;padding:32px}@media(max-width:700px){.investor-table table,.investor-table thead,.investor-table tbody,.investor-table tr,.investor-table td{display:block;width:100%}.investor-table thead{display:none}.investor-table tr{border:1px solid var(--border,#ddd);border-radius:12px;margin:0 0 12px;padding:8px}.investor-table td{display:flex;justify-content:space-between;gap:10px;border:0}.investor-table td:before{content:attr(data-label);font-weight:700}.investor-page .card-header{display:flex;gap:12px}.investor-page .card-header button{align-self:flex-start}}';document.head.appendChild(st);})();
