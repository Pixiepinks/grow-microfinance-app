  ...
  try {
    ...
    const handleDisburse = async (event) => {
      ...
      const renderForm = (options) => {
        ...
        let previewOk = false, previewBalanced = false, previewData = null, previewTimer = null, previewAbortController = null, previewValidationErrors = [], previewErrorText = '';
        ...
        const renderPreview=(data,total,net)=>{ const lines=accountItems(data?.journal_lines||data?.journalLines||data?.lines||[]); const validationErrors=accountItems(data?.validation_errors||data?.validationErrors||[]); previewValidationErrors=validationErrors; const balanced=boolFromBackend(data?.balanced ?? data?.is_balanced ?? data?.isBalanced, Math.abs((Number(data?.total_debit??principal)||0)-(Number(data?.total_credit??principal)||0))<0.01); previewBalanced=!!balanced; if(lines.length){ modal.querySelector('#journal-preview').innerHTML=`<div class="table-scroll"><table><thead><tr><th>Type</th><th>Account</th><th>Debit</th><th>Credit</th></tr></thead><tbody>${lines.map(l=>`<tr><td>${escapeHtml(l.entry_type||l.type||l.side||(Number(l.debit||0)>0?'Dr':'Cr'))}</td><td>${escapeHtml(l.account_name||l.accountName||labelAccount(l.account)||l.description||'')}</td><td>${Number(l.debit||0)>0?formatCurrency(l.debit):''}</td><td>${Number(l.credit||0)>0?formatCurrency(l.credit):(Number(l.amount||0)>0&&String(l.entry_type||l.type||l.side).toUpperCase().startsWith('C')?formatCurrency(l.amount):'')}</td></tr>`).join('')}</tbody></table></div><p><strong>Total Debit:</strong> ${formatCurrency(data.total_debit??data.totalDebit??principal)} &nbsp; <strong>Total Credit:</strong> ${formatCurrency(data.total_credit??data.totalCredit??principal)} &nbsp; <strong>Status:</strong> ${balanced?'Balanced':'Unbalanced'}</p>`; } else { modal.querySelector('#journal-preview').innerHTML='<div class="alert error">Backend preview did not return journal lines.</div>'; previewBalanced=false; } };
        ...
        const validate=()=>{ recalc(); const messages=configMessages(); if(isFutureDateOnly(dateEl.value)) messages.unshift('Future disbursement dates are not supported.'); errEl.innerHTML=[...messages.map(m=>`<div class="alert ${m.startsWith('Configure')?'warning':'error'}">${escapeHtml(m)}</div>`), previewErrorText ? `<div class="alert error">${escapeHtml(previewErrorText)}</div>` : ''].join(''); const total=money(selectedCharges.reduce((s,c)=>s+(c.waived?0:money(c.amount)),0)); const net=money(principal-total); const hasValidationErrors=previewValidationErrors.length>0; confirm.disabled = messages.some(m=>!m.startsWith('Configure')) || !previewData || !previewOk || !previewBalanced || hasValidationErrors || net<=0 || !(accountEl.value && dateEl.value && methodEl.value && (!['BANK_TRANSFER','CHEQUE'].includes(methodEl.value) || refEl.value.trim())) || applicationModalActions.dataset.disbursing === 'true'; };
        ...
        const refreshPreview=async()=>{ if(previewAbortController) previewAbortController.abort(); const controller=new AbortController(); previewAbortController=controller; const signal=controller.signal; previewOk=false; previewBalanced=false; previewData=null; previewValidationErrors=[]; previewErrorText=''; validate(); if(!accountEl.value || configMessages().some(m=>!m.startsWith('Configure'))) return; let timer, timedOut=false; try{ modal.querySelector('#journal-preview').innerHTML='<p>Loading backend preview...</p>'; const fundingAccountId=Number(accountEl.value); if(!Number.isFinite(fundingAccountId)||fundingAccountId<=0) throw new Error('Selected funding account must be a positive database ID.'); const payload={disbursement_date:dateEl.value,funding_account_id:fundingAccountId,transaction_method:methodEl.value,reference:refEl.value.trim(),remarks:modal.querySelector('#disburse-remarks').value.trim(),charges:chargesPayload()}; const path=endpoint('loanApplicationDisbursementPreview',{id:applicationId}); console.log("Disbursement preview", { applicationId, path, payload }); timer=setTimeout(()=>{ timedOut=true; controller.abort(); },15000); previewData=await api(path,{method:'POST',body:payload,signal}); previewOk=true; previewErrorText=''; errEl.innerHTML=''; const total=money(chargesPayload().reduce((s,c)=>s+c.amount,0)); renderPreview(previewData,total,money(principal-total)); }catch(e){ if(e?.name==='AbortError' && signal.aborted && !timedOut) return; previewData=null; previewErrorText=previewErrorMessage(e); modal.querySelector('#journal-preview').innerHTML=`<div class="alert error">${escapeHtml(previewErrorText)}</div>`; } finally { if(timer) clearTimeout(timer); if(previewAbortController?.signal===signal) previewAbortController=null; validate(); } };
