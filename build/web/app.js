...
const defaultApiConfig = {
  ...
  endpoints: {
    ...
    loanDisbursementPreview: '/admin/loans/{id}/disbursement-preview',
  ...
  try {
    ...
    const handleDisburse = async (event) => {
      ...
      const renderForm = (options) => {
        ...
        const accounts = accountItems(options.eligible_funding_accounts || options.eligibleFundingAccounts || options.funding_accounts || options.fundingAccounts || []);
        ...
        const loanId = pick(summary, ['loan_id','loanId','id'], pick(app, ['loan_id','loanId'], appId));
        ...
        const normalizeChargeType = (c) => {
          const account = c.destination_gl_account || c.destinationGlAccount || c.gl_account || c.glAccount || c.income_account || c.account || {};
          ...
          return {
            ...
            destinationAccountId: pick(c, ['destination_gl_account_id','destinationGlAccountId','gl_account_id','glAccountId','income_account_id','incomeAccountId'], account.id || rawMappings[id] || rawMappings[c.code] || rawMappings[c.name] || ''),
            destinationAccountLabel: labelAccount(account) || pick(c, ['destination_gl_account_label','destinationGlAccountLabel','gl_account_label','glAccountLabel'], ''),
        ...
        defaultCharges.forEach(dc => {
          ...
          const base = byId.get(String(id)) || normalizeChargeType(dc);
          selectedCharges.push({ ...base, id: base.id || id, amount: money(pick(dc, ['amount','default_amount','defaultAmount'], base.amount)) });
        });
        ...
        let previewOk = false, previewBalanced = false, previewData = null, previewTimer = null;
        ...
        const chargesPayload=()=>selectedCharges.filter(c=>!c.waived).map(c=>({charge_type_id:c.id,amount:money(c.amount)}));
        ...
        const configMessages=()=>{ const msgs=[]; if(!accounts.length) msgs.push('No active funding account is configured.'); if(!chargeTypes.length) msgs.push(requireDoc?'Documentation Charge is required but not configured.':'No disbursement charges are configured.'); if(!chargeTypes.length && !requireDoc) msgs.push(canManageCharges?'Configure Charges: Accounting Settings → Disbursement Charges':'Contact an administrator to configure disbursement charges.'); if(requireDoc && !selectedCharges.some(c=>String(c.code).toUpperCase()==='DOC_FEE'&&!c.waived)) msgs.push('Documentation Charge is required.'); selectedCharges.filter(c=>!c.waived).forEach(c=>{ if(!money(c.amount) || money(c.amount)<=0) msgs.push(`${c.name} amount must be greater than zero.`); if(!c.destinationAccountId && !c.destinationAccountLabel) msgs.push(`${c.name} has no destination GL account.`); }); const total=selectedCharges.reduce((s,c)=>s+(c.waived?0:money(c.amount)),0); if(total>=principal) msgs.push(total>principal?'Total deductions cannot exceed the loan principal.':'Net disbursed amount must be greater than zero.'); return msgs; };
        const renderPreview=(data,total,net)=>{ const lines=accountItems(data?.journal_lines||data?.journalLines||data?.lines||[]); const balanced=boolFromBackend(data?.balanced ?? data?.is_balanced ?? data?.isBalanced, Math.abs((Number(data?.total_debit??principal)||0)-(Number(data?.total_credit??principal)||0))<0.01); previewBalanced=!!balanced; if(lines.length){ modal.querySelector('#journal-preview').innerHTML=`<div class="table-scroll"><table><thead><tr><th>Type</th><th>Account</th><th>Amount</th></tr></thead><tbody>${lines.map(l=>`<tr><td>${escapeHtml(l.entry_type||l.type||l.side||'')}</td><td>${escapeHtml(l.account_name||l.accountName||labelAccount(l.account)||l.description||'')}</td><td>${formatCurrency(l.amount||l.debit||l.credit||0)}</td></tr>`).join('')}</tbody></table></div><p><strong>Total Debit:</strong> ${formatCurrency(data.total_debit??principal)} &nbsp; <strong>Total Credit:</strong> ${formatCurrency(data.total_credit??principal)} &nbsp; <strong>Status:</strong> ${balanced?'Balanced':'Unbalanced'}</p>`; } else { const bank=validAccounts().find(a=>String(a.id)===accountEl.value); modal.querySelector('#journal-preview').innerHTML=`<div class="accounting-grid"><p><strong>Debit: Loan Principal Receivable</strong><br>${formatCurrency(principal)}</p><p><strong>Credit: ${escapeHtml(labelAccount(bank)||'Main Bank Account')}</strong><br>${formatCurrency(net)}</p>${selectedCharges.filter(c=>!c.waived).map(c=>`<p><strong>Credit: ${escapeHtml(c.destinationAccountLabel||c.name)}</strong><br>${formatCurrency(c.amount)}</p>`).join('')}</div><p><strong>Total Debit:</strong> ${formatCurrency(principal)} &nbsp; <strong>Total Credit:</strong> ${formatCurrency(principal)} &nbsp; <strong>Status:</strong> Balanced</p>`; previewBalanced=true; } };
        ...
        const validate=()=>{ recalc(); const messages=configMessages(); if(isFutureDateOnly(dateEl.value)) messages.unshift('Future disbursement dates are not supported.'); errEl.innerHTML=messages.map(m=>`<div class="alert ${m.startsWith('Configure')?'warning':'error'}">${escapeHtml(m)}</div>`).join(''); const total=money(selectedCharges.reduce((s,c)=>s+(c.waived?0:money(c.amount)),0)); const net=money(principal-total); confirm.disabled = messages.some(m=>!m.startsWith('Configure')) || !previewOk || !previewBalanced || net<=0 || !(accountEl.value && dateEl.value && methodEl.value && (!['BANK_TRANSFER','CHEQUE'].includes(methodEl.value) || refEl.value.trim())) || applicationModalActions.dataset.disbursing === 'true'; };
        const refreshPreview=async()=>{ previewOk=false; previewBalanced=false; validate(); if(!accountEl.value || configMessages().some(m=>!m.startsWith('Configure'))) return; try{ modal.querySelector('#journal-preview').innerHTML='<p>Loading backend preview...</p>'; const payload={disbursement_date:dateEl.value,funding_account_id:accountEl.value,transaction_method:methodEl.value,reference:refEl.value.trim(),remarks:modal.querySelector('#disburse-remarks').value.trim(),charges:chargesPayload()}; previewData=await fetchWithTimeout(endpoint('loanDisbursementPreview',{id:loanId}),{method:'POST',body:payload},15000); previewOk=true; const total=money(chargesPayload().reduce((s,c)=>s+c.amount,0)); renderPreview(previewData,total,money(principal-total)); }catch(e){ errEl.innerHTML=`<div class="alert error">${escapeHtml(disbursementErrorMessage(e))}</div>`; modal.querySelector('#journal-preview').innerHTML='<div class="alert error">Backend preview failed. Correct the errors and retry.</div>'; } finally { validate(); } };
        const schedulePreview=()=>{ clearTimeout(previewTimer); previewTimer=setTimeout(refreshPreview,300); };
        const renderAccounts=()=>{ const keep=accountEl.value; accountEl.innerHTML='<option value="">Select funding account</option>'+validAccounts().map(a=>`<option value="${escapeHtml(a.id)}">${escapeHtml(((a.code||a.account_code||'')+' — '+(a.name||a.account_name||'')).trim())}</option>`).join(''); if([...accountEl.options].some(o=>o.value===keep)) accountEl.value=keep; validate(); schedulePreview(); };
